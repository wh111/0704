	...
	(function() {
		...
		var filterWin = function(v) {
			...
			! frameset && setStyle(ym_win, v == 1 && isCompat ? {
				...
				filter: isIE?'':'Alpha(opacity=' + v * 100 + ')', //zyc修改弹出层后再弹出层背景透明。
				opacity: v
			});
		};
		/*mousemove事件*/
		var mEvent = function(e) {
			...
			resizeMask(true);
			if(sLeft>0){
				if(rootEl.clientWidth-sLeft<100){sLeft=rootEl.clientWidth-100}
			}else{
				if(ym_win.offsetWidth + getScrollPos()[0]+sLeft<100){sLeft=100-ym_win.offsetWidth + getScrollPos()[0];}
			}			
			if(sTop<0){sTop=0}else{if(rootEl.clientHeight-sTop<30){sTop=rootEl.clientHeight-30;}}
			setStyle(ym_wins, {
				left: sLeft + 'px',
				top: sTop + 'px'
			});
		...
		var winVisible = function(visible) {
			...
			var ifmStyle = 'position:absolute;width:100%;height:100%;top:0;left:0;';
			if(curCfg.iframe){top.$._crateLoading();} // iframe加载前创建loading加载进度！
			ym_body.innerHTML = !curCfg.iframe ? ('<div class="' + curCfg.msgCls + '">' + curCfg.message + '</div>') : "<iframe style='" + ifmStyle + "' onreadystatechange='stateChangeIE(this)' onload='stateChangeFirefox(this)' border='0' id='ymIframe' id='ymIframe' name='ymIframe' frameborder='0' src='" + curCfg.message + "'></iframe><div style='" + ifmStyle + ";display:none'></div>"; //内容
			(function(el, obj) {
		...
		ymPrompt.apply(ymPrompt, {
			...
			doHandler: function(sign, autoClose, closeFirst) {
				...
				try { (curCfg.handler)(sign)
					...
					destroy();
					//alert(e.message) //不能执行已释放的Jscript代码
				};
			},

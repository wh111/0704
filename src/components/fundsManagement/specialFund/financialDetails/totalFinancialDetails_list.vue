  ...
  <div ref="content" class="financialDetails">
    ...
    <div id="myTable" ref="myTable">
      <el-table ref="multipleTable" align="center" :height="tabHeight" :context="self" :data="tableData"
        ...
        <el-table-column align="right" prop="accountingMoney" label="实际总额（元）" show-overflow-tooltip>
          <template scope="scope">
      ...
      <div class="total">
        <span>总计：</span>   <span>{{temp.totalOccurrenceMoney | money}} </span> <span style="padding-right: 35px">{{temp.accountingMoney | money}}</span>
      </div>
    </div>
...
<script>
  ...
  export default {
    data () {
      return {
        temp:{accountingMoney:0,totalOccurrenceMoney:0 },
        typeOption,
        deriveModal: false,
        searchMore: false,
    ...
    methods: {
      ...
      getDeriveURL () {
        ...
        this.exportUrl = '/api/' + api.totalExcelExport.path + sltData
      },
      ...
      listDataSuccess (res, m, loading) {
        ...
        let temp={accountingMoney:0,totalOccurrenceMoney:0,outOrEnter:'总计：'}
        if (res.data && res.data.length) {
          res.data.map((item) => {
            item.id = item.accountingId;
            if(item.outOrEnter=='OUT'){
              temp.accountingMoney-=item.accountingMoney;

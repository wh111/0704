  ...
  <div>
    <el-row>
      <el-form class="editForm" :model="formValidate" ref="formValidate" :rules="rules" label-width="100px">
        ...
        <el-col :span="12">
          <el-form-item label="经办人:" prop="userName">{{formValidate.userName}}
        ...
        <el-col :span="12">
          <el-form-item label="经费来源:" prop="fundSource">
            <el-select v-model="formValidate.fundSource" placeholder="请选择">
        ...
        <el-col :span="12">
          <el-form-item label="入账时间:" prop="expenseTime">
            <el-date-picker name="start" v-model="formValidate.expenseTime" :editable="false" type="datetime"
                            placeholder="选择时间" @change="handleExpenseTime"></el-date-picker>
          </el-form-item>
        </el-col>
        ...
        <!--<el-col :span="8">-->
          <!--<el-form-item label="实报总额(元):" prop="accountingMoney">-->
            <!--￥{{formValidate.accountingMoney}}-->
          <!--</el-form-item>-->
        <!--</el-col>-->
        <!--<el-col :span="8">-->
          <!--<el-form-item label="发生总额(元):" prop="accountingMoney">-->
            <!--￥{{formValidate.totalOccurrenceMoney}}-->
          <!--</el-form-item>-->
        <!--</el-col>-->
        <!--<el-col :span="8">-->
          <!--<el-form-item label="单据总数量:" prop="accountingMoney">-->
            <!--{{formValidate.invoiceQuantity}}-->
          <!--</el-form-item>-->
        <!--</el-col>-->
      </el-form>
      <el-col>
      ...
      <el-table align="center" :maxHeight="300" :data="formValidate.fundAccountingDetialList" show-summary
        ...
        <!--<el-table-column class-name="valiTableStyle" prop="projectName" label="项目名称">
          <template scope="scope">
            <el-form :model="{projectName:scope.row.projectName}" ref="f" :rules="rules2"  label-width="0" style="display: inline-block">
        ...
        <el-table-column prop="recordType" label="记录类型">
          <template scope="scope">
            <el-select v-model="scope.row.recordType" placeholder="请选择">
              <el-option v-for="(item,index) in recordType" :key="index" :label="item.value" :value="item.code">
              </el-option>
            </el-select>
          </template>
        </el-table-column>
        <el-table-column prop="invoiceQuantity" label="单据数量">
          <template scope="scope">
            <el-input v-model="scope.row.invoiceQuantity" :maxlength="6" @blur="checkIntegerVal(scope.row,'invoiceQuantity')" @keyup.native="checkIntegerVal(scope.row,'invoiceQuantity')"></el-input>
          </template>
        </el-table-column>
        <el-table-column prop="money" label="实际发生金额">
          <template scope="scope">
            <el-input v-model="scope.row.money" :maxlength="10" @blur="checkMoneyVal(scope.row,'money')" @keyup.native="checkMoneyVal(scope.row,'money')"></el-input>
          </template>
        </el-table-column>
        <el-table-column prop="occurrenceMoney" label="发生金额">
          <template scope="scope">
            <el-input v-model="scope.row.occurrenceMoney" :maxlength="10" @blur="checkMoneyVal(scope.row,'occurrenceMoney')" @keyup.native="checkMoneyVal(scope.row,'occurrenceMoney')"></el-input>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="120">
          <template scope="scope">
            <el-button type="danger" size="small" icon="delete"
                       v-if="formValidate.fundAccountingDetialList.length > 1"
                       @click="removeTable(scope.$index)"></el-button>
            <el-button type="success" size="small" icon="plus" v-if="scope.$index == formValidate.fundAccountingDetialList.length - 1" @click="addTable"></el-button>
          </template>
        </el-table-column>
      </el-table>
...
<script>
  ...
  import {detailRules as rules2} from '../../rules';
    ...
    methods: {
      ...
      getSummaries(param) {
        ...
        if (typeof sums[3] == 'number') {
          sums3 = (sums[3]).toFixed(2)
          sums[3] = '￥ '+(sums[3]).toFixed(2)
        }
        if (typeof sums[4] == 'number') {
          sums4 = (sums[4]).toFixed(2)
          sums[4] = '￥ '+(sums[4]).toFixed(2)
        }
        this.formValidate.invoiceQuantity = sums[2];
        this.formValidate.accountingMoney = sums3;
        this.formValidate.totalOccurrenceMoney = sums4;
        return sums;
      },
      /****************** 其他逻辑 *****************/
      // 获取经费报销类型
      ...
      getRecordType() {
        let opt = {
          ajaxSuccess: res => this.recordType = res.data || [],
      ...
      getEditData() {
        let opt = {
          ajaxSuccess: res => {
            ...
            res.data['fundAccountingDetialList'].map((item,index)=>{
              item['money'] = item['money'].toFixed(2);
              item['occurrenceMoney'] = item['occurrenceMoney'].toFixed(2);
            })
      ...
      checkTableData() {
        for (let i = 0, d = this.formValidate.fundAccountingDetialList, l = d.length; i < l; i++) {
          ...
          if (!d[i].recordType) {
            this.errorMess(`第 ${i + 1} 行的“记录类型”必须填写`);
            return false
          }
          if (!/^\+?[1-9]\d*$/.test(d[i].invoiceQuantity)) {
            this.errorMess(`第 ${i + 1} 行的“单据数量”必须正整数且大于零`);
            return false
          }
          if (d[i].money == '') {
            this.errorMess(`第 ${i + 1} 行的“实报金额”不能为空!`);
            return false
          }
          if (d[i].occurrenceMoney == '') {
            this.errorMess(`第 ${i + 1} 行的“实报金额”不能为空!`);
            return false
          }
          if (!/^(0|[1-9](\d+|))(\.\d{1,2}|)$/.test(d[i].money)) {
            this.errorMess(`第 ${i + 1} 行的“实报金额”必须是有效的整数或者二位小数`);
            return false
          }
          if (!/^(0|[1-9](\d+|))(\.\d{1,2}|)$/.test(d[i].occurrenceMoney)) {
            this.errorMess(`第 ${i + 1} 行的“发生金额”必须是有效的整数或者二位小数`);
            return false
          }
          if(Number(d[i].money) > Number(d[i].occurrenceMoney)){
            this.errorMess(`第 ${i + 1} 行的“实报金额不能大于发生金额!”`);
            return false
          }
        }
        return true
      ...
      /**
       * 检查数据的实报金额、发生金额是否为正数，如果有小数则小数点后的位数不能大于两位
       * @param row object 当前添加行的数据对象
       * @param key string 当前添加行的数据KEY属性名称
       */
      checkMoneyVal(row,key){
        let val = row[key];
        if(!(/^\d+\.?\d*$/m.test(val))){
          row[key] = "";
        }else{
          if (!/^\d+\.?\d{0,2}$/m.test(val)) {
            let str = row[key]+""
            let strSplit = str.split(".")
            row[key] = strSplit[0]+'.'+strSplit[1].substring(0,2);
          }
        }
      },
       ...
       * 检查单据数量是否为正整数
       */
      checkIntegerVal(row,key){
        let val = row[key];
        if(!/^\+?[0-9]\d*$/.test(val)){
          let str = row[key]+""
          row[key] = str.substring(0,str.length-1);
        }
      },

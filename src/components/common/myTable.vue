  ...
  <div :class="wrapClasses" :style="styles">
    <div :class="classes">
      ...
      <div :class="[prefixCls + '-header']" v-if="showHeader" ref="header" @mousewheel="handleMouseWheel">
        <table-head :prefix-cls="prefixCls" :styleObject="tableStyle" :columns="cloneColumns" :obj-data="objData" :columns-width="columnsWidth"
          :data="rebuildData"></table-head>
      </div>
      <div :class="[prefixCls + '-body']" :style="bodyStyle" ref="body" @scroll="handleBodyScroll" v-show="!((!!noDataText && (!data || data.length === 0)) || (!!noFilteredDataText && (!rebuildData || rebuildData.length === 0)))">
        <table-body ref="tbody" :height="height" :prefix-cls="prefixCls" :styleObject="tableStyle" :columns="cloneColumns" :data="rebuildData"
          :columns-width="columnsWidth" :obj-data="objData"></table-body>
      </div>
      <div :class="[prefixCls + '-tip']" v-show="((!!noDataText && (!data || data.length === 0)) || (!!noFilteredDataText && (!rebuildData || rebuildData.length === 0)))">
      ...
      <div :class="[prefixCls + '-fixed']" :style="fixedTableStyle" v-if="isLeftFixed">
        <div :class="[prefixCls + '-fixed-header']" v-if="showHeader">
          <table-head fixed="left" :prefix-cls="prefixCls" :styleObject="fixedTableStyle" :columns="leftFixedColumns" :obj-data="objData"
            :columns-width.sync="columnsWidth" :data="rebuildData"></table-head>
        </div>
      ...
      <div :class="[prefixCls + '-fixed-right']" :style="fixedRightTableStyle" v-if="isRightFixed">
        <div :class="[prefixCls + '-fixed-header']" v-if="showHeader">
          <table-head fixed="right" :prefix-cls="prefixCls" :styleObject="fixedRightTableStyle" :columns="rightFixedColumns" :obj-data="objData"
  ...
  export default {
    ...
    computed: {
      ...
      styles() {
        let style = {};
        if (this.height) {
          const height = (this.isLeftFixed || this.isRightFixed) ? parseInt(this.height) + this.scrollBarWidth :
      ...
      tableStyle() {
        ...
        // (this.cloneColumns[0].length*150<this.tableWidth)
      ...
      bodyStyle() {
        ...
        if (this.bodyHeight !== 0) {
          ...
          //   alert(this.bodyHeight + "---------" + this.scrollBarWidth + "---------" + this.bodyHeight)
          style.height = `${height}px`;
        }
        return style;
      },
      ...
      handleResize() {
        this.$nextTick(() => {
          ...
          this.$nextTick(() => {
            ...
            try {
              if (this.data.length) { // todo 修改了源码，使this.cloneColumns变为this.cloneColumns[this.cloneColumns.length-1]
                const $td = this.$refs.tbody.$el.querySelectorAll('tbody tr')[0].querySelectorAll('td');
                for (let i = 0; i < $td.length; i++) { // can not use forEach in Firefox
                  const column = this.cloneColumns[this.cloneColumns.length - 1][i];
                  let width = parseInt(getStyle($td[i], 'width'));
                  if (i === autoWidthIndex) {
                    width = parseInt(getStyle($td[i], 'width')) - 1;
                  }
                  if (column.width) width = column.width;
                  this.cloneColumns[this.cloneColumns.length - 1][i]._width = width;

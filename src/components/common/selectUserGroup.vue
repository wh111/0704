...
<script>
  ...
  let Util = null;
  export default {
    props: ["initUser", 'isOnlyOne', 'url', 'unSelect','treeUrl','selectOption'],
    data() {
      return {
        //tree默认项设置
        treeDefaults: {
    ...
    methods: {
      ...
      searchLeafData(id) {
        let userList = {
          ajaxSuccess: (res) => {
            ...
            for (var i = 0, item; i < rawData.length; i++) {
              item = rawData[i];
              //过滤掉未通过报名审核的
              if(item.auditStatus!=='AUDIT_SUCCESS'){
                continue;
              }
        ...
        if(this.selectOption){
          userList.ajaxParams = this.selectOption;
          if(!userList.ajaxParams.params)userList.ajaxParams.params={};
          userList.ajaxParams.params.deptIds = id;
        };
        this.ajax(userList);
      },

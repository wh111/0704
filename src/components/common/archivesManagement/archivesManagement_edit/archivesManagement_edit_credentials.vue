            ...
            <el-table-column
              ...
              <template scope="scope">
                <el-form-item prop="certificateName">
                  <el-input style="width: 100%;" v-model="scope.row.certificateName"  placeholder="请输入内容"></el-input>
                </el-form-item>
              </template>
            </el-table-column>
            <el-table-column
    ...
    data (){
      return{
        ...
        editMessTitle:{
          ...
          ajaxError:'ajaxError',
          ajaxParams:{
            url:'/archives/save/qualified/'+this.dataId,
            method:'post',
...
//          this.$emit("setSaveData",this.editMessTitle.ajaxParams.data);
      ...
      getFormData(data){
        ...
        let tempArr = [];
        let valValue = ["certificateName","certificateNo","obtainDate","remark"]
        for(var i=0;i<myData.length;i++){
          if(!this.objValIsEmpty(myData[i],valValue)){
            tempArr.push(myData[i]);
          }
        }
        tempArr = this.formDate(tempArr,['obtainDate'],this.yearMonthData);
        return tempArr;
      },

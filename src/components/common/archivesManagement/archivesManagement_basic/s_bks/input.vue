<template>
  <!-- 基本信息 - 本科生 - 填写 -->
  <el-form ref="formValidate" :rules="rules" :model="formValidate" class="demo-form-inline editForm" label-width="115px">
    <!--标题-->
    <el-row>
...
<script>
  ...
  import api from '../../../../educationManagement/EnrollEnroll/bksPersonnelManagement/api';
  import {pub,bks} from '../rules.js';
  //上传头像组件引入
  ...
  export default {
    ...
    methods: {
       ...
       * */
      init() {
        this.rules = Util._.defaultsDeep({},pub,bks,this.addRules || {});
        //初始化数据
        if(this.dataId){
          this.getViewData()
        }else{

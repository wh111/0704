<template>
  <!-- 基本信息 - 实习生 - 填写 -->
  <el-form ref="formValidate" :rules="rules" :model="formValidate" class="demo-form-inline editForm"
           label-width="115px">
    <!--标题-->
    <el-row>
            ...
            <el-row :gutter="10" class="table-back-one">
              <el-col :span="8">
                ...
                <el-form-item label="姓名：" prop="name">
                  <span v-if="formWhere && formWhere == 'work'">{{formValidate.name}}</span>
                  <span
                    v-else-if="formValidate.auditStatus=='AUDIT_SUCCESS'||formValidate.auditStatus=='NOT_AUDIT'">{{formValidate.name}}</span>
                ...
                <el-form-item label="专业：" prop="specialty">
                  <el-select v-model="formValidate.specialty" placeholder="请选择">
                    <selectOption type="getByCode" unAll="true" isCode="true" id="value"
                                  codeType="TRAINING_DIRECTION_SX"></selectOption>
                  </el-select>
            ...
            <el-row :gutter="10" class="table-back-two">
              <el-col :span="8">
                <el-form-item label="实习开始日期：" prop="startDate" label-width="124px">
                  <span v-if="formWhere && formWhere == 'work'">{{formValidate.startDate}}</span>
              ...
              <el-col :span="8">
                <el-form-item label="实习期：" prop="internshipNum">
                  <span v-if="formWhere && formWhere == 'work'">{{formValidate.internshipNum}}个月</span>
                  <el-input v-else v-model="formValidate.internshipNum" placeholder="（月为单位）"></el-input>
                </el-form-item>
              </el-col>
              <el-col :span="8">
              </el-col>
...
<script>
  ...
  export default {
    //props接收父组件传递过来的数据
    // addRules 追加的验证规则
    props: ['operailityData', 'addRules', 'dataId', 'mustUploadHP', 'formWhere'],
    data () {
      return {
        url: pubApi,
        rules: {},
    ...
    methods: {
       ...
       * */
      init () {
        this.rules = Util._.defaultsDeep({}, pub, sxs, this.addRules || {});
        //初始化
        if (this.dataId) {
          this.getViewData()
        } else {
...
//            }
      ...
      SuccessGetCurrData (data) {
        ...
        if (data.auditStatus == 'AUDIT_FAILURE') {
          let opt = {
            ajaxSuccess: res => {

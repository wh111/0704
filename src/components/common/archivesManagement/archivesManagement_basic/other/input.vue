<template>
  <!-- 基本信息 - 其他人员 - 填写 -->
  <el-form ref="formValidate" :rules="rules" :model="formValidate" class="editForm archivesBasicInput" label-width="115px">
    <!--标题-->
    <el-row>
      ...
      <el-col :span="24">
        <el-row :gutter="10">
            ...
            <el-row :gutter="10" class="table-back-one">
              ...
              <el-col :span="8">
                <el-form-item label="学校：" prop="school">
                  ...
                  <!--<search-select @setSltOptionValue="setSltOptionValue" :selectOptions="selectOptions" :selVal="schoolId"></search-select>-->
                </el-form-item>
              </el-col>
            </el-row>
          ...
          <el-col :span="4" class="table-back-header">
              <up-header @upladSuccess="setUploadHeaderSuccessUrl" :imgFile="imgSrc" :actionUrl="uploadHeaderUrl" :avatarTips="true"></up-header>
          </el-col>
        </el-row>
        ...
        <el-row :gutter="10" class="table-back-two-d">
          <el-col>
            <el-form-item label="来源单位：" prop="fromHospital">
              <el-input v-model="formValidate.fromHospital" placeholder="请输入单位名称"></el-input>
            </el-form-item>
          </el-col>
        </el-row>
...
<script>
  import api from '../api';
  import {pub,other} from '../rules.js';
  //上传头像组件引入
  ...
  export default {
    ...
    data() {
      return {
        ...
        //上传头像
        uploadHeaderUrl: api.uploadHeaderUrl,
        //是否显示头像
        imgSrc: '',
        ...
        editMessTitle:{
          ...
          ajaxError:'ajaxError',
          ajaxParams:{
            url:api.otherSave.path,
            method:api.otherSave.method,
            data:{}
          }
        },
      }
    ...
    created() {
      ...
      //初始化
      this.init();
      if (this.operailityData && this.operailityData.school != 'undefined') {
        this.operailityData.school=this.operailityData.schoolName;
      }
      this.uploadHeaderUrl = api.uploadHeaderUrl + '?userId=' + (this.operailityData && this.operailityData.id);
    },
    methods: {
      /*
       * 组件初始化入口
       * */
      init() {
        this.rules = Util._.defaultsDeep({},pub,other,this.addRules || {});
        //初始化数据
        if(this.dataId){
          this.getViewData()
        }else{
      ...
      getDictionaryForPassWorld(){
        let opt = {
          ...
          ajaxParams: {
            url: api.getDictionary + 'PASSWORD',
            method: 'get'
          }
        }
        this.ajax(opt)
      ...
      SuccessGetCurrData(data) {
       // let data = this.initData; //this.$store.state.archivesAudit.index.archivesContent.archivesBasicInfoDto;//this.initData;//responseData.data;
        ...
        this.formValidate.school=this.formValidate.schoolName;
      },

...
<template>
  <div class="depContainer my-leftTree" :class="defaults.cls">
    ...
    <div v-if="defaults.isShowMenus" class="treeMenus" style="">
      ...
      <!--:disabled="sltParentId===0?true:false" -->
    </div>
    <div class="treeContent" :style="defaults.selectUser?'top:36px':''">
      <el-tree :style="style" highlight-current node-key="id" class="filter-tree" :data="defaults.treeData "
               :props="defaultProps" @node-click="treeClick" :current-node-key="currentNodeKey"
               :default-expanded-keys="expandedKeys" :expand-on-click-node="false"
               :empty-text="defaults.emptyText" :lazy="defaults.asyn" :render-content="renderContent"
               @node-expand="getOpenNode" @node-collapse="nodeCollapse" :load="loadNode" ref="treeId">
        <i class="el-icon-delete"></i>
      </el-tree>
    </div>

    <!--新建弹窗-->
    <Modal :mask-closable="false" v-model="addModal" height="200" title="对话框标题" class-name="vertical-center-modal" :width="560">
      <modal-header slot="header" :content="addId"></modal-header>
      <template>
        <add-custom v-if="addModal" @cancel="cancel" @add="subCallback" :operaility-data="operailityData"></add-custom>
      </template>
      <!--<template v-else>-->
      <!--<add-dep v-if="addModal" type="add" @cancel="cancel" @add="subCallback"-->
      <!--:operaility-data="operailityData"></add-dep>-->
      <!--</template>-->
      <div slot="footer"></div>
    </Modal>
    <!--修改弹窗-->
    <Modal :mask-closable="false" v-model="editModal" height="200" title="对话框标题" class-name="vertical-center-modal" :width="560">
      ...
      <template v-else>
        <edit-dep v-if="editModal" type="edit" @cancel="cancel" @edit="subCallback"
                  :operaility-data="operailityData"></edit-dep>
      </template>
      <div slot="footer"></div>
    </Modal>
    <!--删除弹窗-->
    <Modal :mask-closable="false" v-model="removeModal" height="200" title="对话框标题" class-name="vertical-center-modal" :width="500">
...
<script>
  import addDep from './addAssessment.vue';
  import editDep from './editAssessment.vue';

  import addCustom from './addAssessment'; // 自定义增加节点
  import editCustom from './editAssessment'; // 自定义编辑节点
  ...
  export default {
    ...
    props: ['treeOptions', 'fromWhereTreeType', 'clickAddChange', 'initSltedNode', 'currentKey', 'upDateNumber', 'comparisonAssignment'],
    data () {
      return {
        //tree
        defaults: {
          getTreeUrl: '/examination/achievetree/tree', //目录树结构请求地址
          baseUrl: '/examination/achievetree',
          asyn: false, //是否异步加载
        ...
        fromWhereTree: this.fromWhereTreeType, //哪里用的目录树   user  questionBank custom(自定义节点)
    ...
    methods: {
      ...
      init () {
        ...
        // 自定义
        //if (this.fromWhereTree === 'custom') { /examination/achievetree/remove/
      ...
      getTreeData (responseData) {
        ...
        //   if (this.fromWhereTree == "undefined") {
      ...
      //  * */
      // setCurrTreeType() {
      //   if (this.fromWhereTree == "teach_bkjy") {
      ...
      <
        i
      class
        = 'ivu-icon ivu-icon-android-folder ambuf-tree-icon' > < /i>;;
        if (node.isLeaf) {
          icon =
        <
          i
        class
          = 'ivu-icon ivu-icon-document ambuf-tree-icon' > < /i>;;
        } else {
          if (node.expanded) {
            icon =
          <
            i
          class
            = 'ivu-icon ivu-icon-android-folder-open ambuf-tree-icon' > < /i>;;
          }
        }
        ...
        /span></span > < span
        style = 'float:right;margin-right:20px' > < /span></span >
      )
      },
      //新建部门、修改部门、删除部门
      operationTreeData (type) {
        // if (this.fromWhereTree == "undefinde") {
        ...
        this.operailityData = Object.assign({}, this.sltedTreeData, this.baseUrl, {
          ...
          fromWhereTre: this.fromWhereTreeType
        });

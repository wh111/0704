...
<template>
  <div id="content" ref="content" class="modal">
      ...
      <div v-show="searchMore" ref="searchMore" align="right">
        ...
        <el-form-item label="状态" prop="status">
          <el-select filterable v-model="formValidate.status" placeholder="请选择">
            ...
            <el-option label="全部" value=""></el-option>
            <el-option label="待审核" value="JYSDSH"></el-option>
            <el-option label="不通过" value="JYSBH"></el-option>
            <el-option label="通过" value="JYCDSH"></el-option>
      ...
      >
        <el-table
          ...
          <el-table-column
            ...
            width="80">
            <template scope="scope">
              <el-button size="small" :disabled="scope.row.status != 'JYSDSH'" @click="audit(scope.row)">审核</el-button>
            </template>
          </el-table-column>
          <el-table-column
            show-overflow-tooltip
    ...
    <Modal
      ...
      <operate v-if="passModal" :type="'pass'" :operate-url="url.diseaseRecordManageModifySubPass" :methods="'post'" :operateData="passData" @cancel="cancel"
               @operate="subCallback" :operaility-data="operailityData"></operate>
      <div slot="footer"></div>
    </Modal>
    ...
    <Modal
      ...
      <operate v-if="rejectModal" :type="'reject'" :operate-url="url.diseaseRecordManageModifySubReject" :operateData="rejectData" :methods="'post'"
    ...
    data () {
      return {
        ...
        passData: {
          ...
          method: this.url.jysAudit.method,
          data: {status: 'JYCDSH', jysAuditOpinion: '通过'}
        },
        rejectData: {
          url: this.url.jysAudit.path,
          method: this.url.jysAudit.method,
          data: {status: 'JYSBH', jysAuditOpinion: '驳回'}
        },
        style: {
          publish: false
        },
        ...
        listMessTitle: {
          ...
          ajaxParams: {
            url: this.url.jysAuditList.path,
            params: {}
          }
        },
        /*--按钮button--*/

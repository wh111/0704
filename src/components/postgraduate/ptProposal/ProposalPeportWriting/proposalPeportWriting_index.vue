...
----------------------------------->
<template>
  <div style="height: 100%;" ref="content" class="modal">
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template scope="scope">
              ...
              <el-button size="small" type="warning" :disabled="!['WSB','DSBH','JYSBH','JYCBH',''].includes(scope.row.status)" @click="edit(scope.row)">修改
              </el-button>
              <el-button size="small" type="danger" :disabled="!['WSB','DSBH','JYSBH','JYCBH',''].includes(scope.row.status)" @click="reported(scope.row)">上报
              </el-button>
            </template>
          </el-table-column>
          <el-table-column
    ...
    <Modal
      ...
      <edit v-if="addModal" @cancel="cancel" @add="subCallback" :url="url"></edit>
      <div slot="footer"></div>
    </Modal>
    <!--修改-->
    <Modal
    ...
    <Modal
      ...
      <paw-select v-if="selectModal" @select="selectPaw" @cancel="cancel"></paw-select>
      <div slot="footer"></div>
    </Modal>
  </div>
</template>
<script>
  ...
  import url                                       from './api';
  //引入--新建--组件
  //  import add                                       from './ProposalPeportWriting_add.vue';
  //引入--修改--组件
  //  import edit                                      from './ProposalPeportWriting_edit.vue';
  //  import edit                                      from './proposalPeportWriting_input.vue';
  //引入--查看--组件
  import show                                      from './ProposalPeportWriting_view.vue';
  import pawSelect                                 from '../proposalApplicationWrite/proposalApplicationWrite_select';
...
//        this.openModel('add');
      ...
      selectPaw (obj) {
        // 根据开题申请去写开题报告
        let {id: applicationId, thesisTitle: topicName, researchDirection, deptId} = obj[0];
        this.cancel('select');
        this.$emit('show', 'input', {applicationId, topicName, researchDirection, deptId});
      },
      /*--点击--发布--按钮--*/
      reported (data) {

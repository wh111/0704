<template>
  ...
  <div>
    ...
    <el-row class="but-space">
      ...
      <el-col :span="16">
        <p>上传填好的成绩表 ( 仅支持.xls/.xlsx格式)</p><br>
    ...
    <Modal
      ...
      <to-channel v-if="toChannelModal" @cancel="closeChannel" url="/enroll/person/excelImport/score"
                  @success="subHandelEvent" :data="convertedData" :format="format" :addData="addData"></to-channel>
      <div slot="footer"></div>
    </Modal>
  </div>
</template>
  ...
  export default {
    ...
    data() {
      return {
        ...
        toChannelId: {id: 'toChannelId', title: '人员成绩表'},
        convertedData: {},
        format: {
        ...
        addMessTitle: {
          ...
          ajaxError: 'ajaxError',
          ajaxParams: {
            url: 'enroll/person/excelImport/score',
            method: 'post'
          }
        },
        //table
    ...
    mounted() {
      ...
      this.http = http + 'static/template/研究生拟录取成绩表导入模板.xls';
      this.init();
    },
    methods: {
      /*
       ...
       * */
      success() {
        this.$emit('ranking', 'toChannel', `导入${this.totalNumber}条数据`);
      },

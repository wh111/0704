...
<template>
  <div>
    <el-form :model="formValidate" ref="formValidate" :rules="rules">
        ...
        <el-col>
          <el-form-item label="复试专业:" prop="majorCode">
            <el-select filterable v-model="formValidate.majorCode" @change="majorChange" placeholder="请选择">
              <major unAll="true" @successMajor="getMajorSuccess"></major>
            </el-select>
          </el-form-item>
        </el-col>
        <el-col>
      ...
      <el-row>
        <el-col>
          <el-form-item label="已选中人员:" prop="userIds" label-width="100px">
            <template v-if="selectPerson && (selectPerson instanceof Object)">
        ...
        <el-col>
          <el-form-item label="复试地点:" prop="address" label-width="100px">
            <el-input v-model="formValidate.address" placeholder="请输入复试地点"></el-input>
          </el-form-item>
        </el-col>
        <el-col>
          <el-form-item label="复试时间:" prop="time" label-width="100px">
        ...
        <el-col>
          <el-form-item label="注意事项:" prop="require" label-width="100px">
...
<script>
  ...
  let Util = null;
  export default {
    props: ['operailityData', 'noticeData'],
    data () {
      return {
        rules,
        //保存按钮基本信息
    ...
    methods: {
      ...
      init () {
        ...
        if (this.operailityData) {
          ...
          this.formValidate.batch = this.noticeData.id;
        }
        //ajax请求参数设置
...
//        this.getMajor()
      ...
      //获取专业数据成功
      getMajorSuccess (data) {
        let t = {};
        data.map(item => {
          item.value = item.major;
          t[item.majorCode] = item;
        });
        this.majorData = t;
        this.formValidate.majorCode = data[0] && data[0].majorCode;
      },
      // 获取编辑数据
      getEditData () {
        let opt = {
          ajaxSuccess: res => {
            ...
            this.selectPerson = this.setSelectPerson(this.oldData.userList);
            /* // 附件处理
             let fileId = [];
             let fileList = [];

...
<template>
  <div id="content" ref="content">
    <!--<div id="content" ref="content" v-if="typeView.includes('DS') || typeView.includes('KEZR') || typeView.includes('YJSGL') || typeView.includes('JYC')">-->
    <el-form ref="formValidate" :model="formValidate" inline>
      <el-row style="margin-bottom:0">
        <!--列表操作按钮-->
        <el-col :span="10">
          <el-button type="success" :disabled="isPass" @click="pass(1)">答辩通过</el-button>
          <el-button type="danger" :disabled="isPass" @click="reject(2)">答辩不通过</el-button>
          <!--<el-button v-if="typeView.includes('DS')" type="primary" :disabled="isPass" @click="pass(1)">答辩通过</el-button>-->
          <!--<el-button v-if="typeView.includes('DS')" type="danger" :disabled="isPass" @click="reject(2)">答辩不通过</el-button>-->
          <!--<el-button v-if="['KEZR'].includes(typeView)" type="primary" :disabled="isPass" @click="affirm(3)">确认</el-button>-->
          <!--<el-button v-if="typeView.includes('YJSGL') || typeView.includes('JYC')" type="primary" :disabled="isPass" @click="affirm(4)">确认-->
          <!--</el-button>-->
          <!--<el-button v-if="isPass" type="primary" @click="auditing('TG')">提示研究生上报毕业申请</el-button>-->
        </el-col>
        <!--搜索项-->
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template scope="scope">
              ...
              <!--<el-button type="primary" :disabled="scope.row.status!='DSH'" size="small" @click="audit(scope.row)">审核</el-button>-->
            </template>
          </el-table-column>
          <!--<el-table-column-->
    ...
    <Modal
      ...
      <operate v-if="passModal" type="pass" :operateData="passData" @operate="subCallback" @cancel="cancel"></operate>
      <div slot="footer"></div>
    </Modal>
    ...
    <Modal
      ...
      <operate v-if="rejectModal" type="reject" :operateData="passData" @operate="subCallback" @cancel="cancel"></operate>
      <div slot="footer"></div>
    </Modal>
    <!--不通过弹窗-->
    <Modal
      ...
      :loading="loading"
      :width="500">
      <modal-header slot="header" :content="affirmId"></modal-header>
      <operate v-if="affirmModal" :type="'affirm'" :operateData="passData" @operate="subCallback" @cancel="cancel"></operate>
      <div slot="footer"></div>
    </Modal>
  </div>
  <!--<div v-else style="font-size: 18px;text-align: center;line-height: 100px;"><strong>您没有此权限</strong></div>-->
</template>
<script>
  /*当前组件必要引入*/
  import audit                        from './respondentResultRecord_audit.vue';
  import show                         from './respondentResultRecord_view.vue';
  ...
  export default {
    data () {
      return {
        ...
        passId: {id: 'passId', title: '答辩通过'},
        rejectId: {id: 'rejectId', title: '答辩不通过'},
        affirmId: {id: 'affirmId', title: '确认'},
        passData: {
          url: api.defenseAppResAudit.path,
          data: {
            ids: '',
            resultStatus: '1'
          }
    ...
    methods: {
      ...
      init () {
        ...
        /*let typeView = [];
         ...
         this.formValidate.resultStatus = '';
         this.options = [
         {
         value: '',
         label: '全部'
         ...
         this.formValidate.resultStatus = '1';
         this.options = [
         {
         value: '1',
         label: '未确认'
...
//            if (this.typeView.includes('DS') && val[i].resultStatus != '0') {
//              this.isPass = true;
//            } else if ((this.typeView.includes('YJSGL') || this.typeView.includes('JYC')) && val[i].resultStatus != '1') {
//              this.isPass = true;
//            }
//          }
//        }
...
//      },
      ...
      pass (index) {
        ...
        this.passData.data.resultStatus = index;
        this.openModel('pass');
      },

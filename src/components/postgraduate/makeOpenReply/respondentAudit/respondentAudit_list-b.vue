...
<template>
  <div id="content" ref="content" class="modal">
      ...
      >
        <el-table
          ...
          <el-table-column
            ...
            width="200">
            <template scope="scope">
              <span v-if="scope.row.status  == 'DSH'" style="color:#7a70bd;cursor: pointer;"  @click="audit(scope.row)">{{scope.row.fileInfo && scope.row.fileInfo.fileName}}</span>
              <span v-else>{{scope.row.fileInfo && scope.row.fileInfo.fileName}}</span>
            </template>
          </el-table-column>
          <el-table-column
            show-overflow-tooltip
    ...
    <Modal
      ...
      <audit  :view="view" v-if="auditModal" @cancel="cancel" @audit="subCallback" :operaility-data="operailityData" :url="url"></audit>
      <div slot="footer"></div>
    </Modal>

...
<template>
  ...
  <div>
    ...
    <Modal
      ...
      <add   @cancel="cancelSetDep" @setUsers="setUsers" :operaility-data="formValidate.summaryUserList"></add>
      <div slot="footer"></div>
    </Modal>
  </div>
</template>
<script>
  ...
  export default {
    data (){
      return{
        ...
        addMessTitle:{
          ...
          ajaxError:'ajaxError',
          ajaxParams:{
            url:api.defenseSummaryAdd.path,
            method:api.defenseSummaryAdd.method,
            jsonString:true,
            data:{},
          }
        },
    ...
    methods:{
      ...
      //保存 改变url
      save(isLoadingFun){
        this.addMessTitle.ajaxParams.url = api.defenseSummaryAdd.path;
        this.addMessTitle.ajaxParams.method = api.defenseSummaryAdd.method;

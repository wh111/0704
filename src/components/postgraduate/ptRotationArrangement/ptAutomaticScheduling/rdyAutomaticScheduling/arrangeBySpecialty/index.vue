...
<template>
  <div>
    <el-steps style="margin: 0 auto; width: 98%;" :space="160" :active="active" finish-status="success">
      ...
      <el-step title="第二步：选择培训标准"></el-step>
      <el-step title="第三步：选择人员"></el-step>
    ...
    <div v-if="active==1" style="margin: 20px;">
      <third @setThirdVal="setThirdVal" :userType="userType" :initRtId="initRtId" :jdId="schoolId"></third>
    </div>
    <!--- 第三步：选择人员 --->
    <div v-if="active==2" style="margin: 20px;">
      <second @setSecondVal="setSecondVal" :userType="userType" :initUser="initUser" :jdProclass="jdProclass" :schoolId="schoolId"></second>
    </div>
    <!--- 第四步：设置轮转时间 --->
    ...
    methods: {
     ...
     //存储三第步的数据
      ...
      next() {
        ...
        if(this.postData["userName"]==""&&this.active==2){
          this.errorMess("请选择人员!");
          return;
        }
        if(this.postData["rtName"]==""&&this.active==1){

  ...
  <div id="content" ref="content" class="modal">
      ...
      >
        <el-table
          ...
          <el-table-column
            ...
            width="100">
            <template scope="scope">
              <el-button size="small" :disabled="scope.row.status != 'AUDIT'" @click="audit(scope.row)">审核</el-button>
            </template>
          </el-table-column>
    ...
    <Modal
      ...
      <operate v-if="passModal" :type="'pass'" :operate-url="url.tutorAudit.path" :methods="'post'"
               :operateData="passData" @cancel="cancel" @operate="subCallback"></operate>
      <div slot="footer"></div>
    </Modal>
    ...
    <Modal
      ...
      <operate v-if="rejectModal" :type="'reject'" :operate-url="url.tutorAudit.path" :operateData="rejectData"
               :methods="'post'" @cancel="cancel" @operate="subCallback"></operate>
      <div slot="footer"></div>
    </Modal>
  </div>
</template>
  ...
  export default {
    ...
    data () {
      return {
        ...
        passData: {
          ids: '',
          data: {status: 'DSADOPT', dsopinion: '通过'},
          method: this.url.tutorAudit.method
        },
        rejectData: {
          ids: '',
          data: {status: 'DSTPASS', dsopinion: '驳回'},
          method: this.url.tutorAudit.method
        },
        style: {
          publish: false

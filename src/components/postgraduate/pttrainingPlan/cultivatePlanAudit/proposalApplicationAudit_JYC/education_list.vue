      ...
      >
        <el-table
          ...
          <el-table-column
            ...
            width="100">
            <template scope="scope">
              <el-button size="small" :disabled="scope.row.status != 'KSZRADOPT'" @click="audit(scope.row)">审核
              </el-button>
            </template>
          </el-table-column>
  ...
  export default {
    ...
    data () {
      return {
        ...
        passData: {
          ids: '',
          data: {status: 'JYCADOPT', jycopinion: '通过'},
          method: this.url.tutorAudit.method
        },
        rejectData: {
          ids: '',
          data: {status: 'JYCTPASS', jycopinion: '驳回'},
          method: this.url.tutorAudit.method
        },
        operailityData: '',
        multipleSelection: [],

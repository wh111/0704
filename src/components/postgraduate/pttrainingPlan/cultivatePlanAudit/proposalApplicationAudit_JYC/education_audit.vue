<template>
  ...
  <div>
    <el-form :model="formValidate" :rules="rules" ref="formValidate" class="demo-form-inline" label-width="90px">
      ...
      <el-row>
        <el-col>
          <el-form-item label="审核意见:" prop="jycopinion">
            <el-input v-model="formValidate.jycopinion" type="textarea"></el-input>
          </el-form-item>
        </el-col>
      </el-row>
      <br>
  ...
  export default {
    ...
    methods: {
      ...
      passSubEvent (isLoadingFun) {
        this.formValidate.status = 'JYCADOPT';
        this.listenSubEvent(isLoadingFun);
      },

      //驳回
      rejectSubEvent (isLoadingFun) {
        this.formValidate.status = 'JYCTPASS';
        this.listenSubEvent(isLoadingFun);

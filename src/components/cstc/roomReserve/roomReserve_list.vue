      ...
      <el-table align="center" :height="tableHeight" :context="self" :data="tableData" tooltip-effect="dark" class="tableShowMoreInfo" style="width: 100%;"
        ...
        <el-table-column label="操作" width="260">
          <template scope="scope">
            ...
            <el-button size="small" type="warning" :disabled="['REPORTED','ADOPT'].includes(scope.row.status)" @click="edit(scope.row)">修改</el-button>
            <el-button size="small" type="success" :disabled="['REPORTED','ADOPT'].includes(scope.row.status)" @click="changStatus(scope.row,'REPORTED')">上报</el-button>
            <el-button size="small" type="danger" :disabled="!['REPORTED'].includes(scope.row.status)" @click="changStatus(scope.row,'UNREPORTED')">撤回</el-button>
          </template>
        </el-table-column>
...
<script>
  ...
  import edit from './roomReserve_input';
  import show from './roomReserve_view';
  //当前组件引入全局的util
  let Util = null;
  export default {
    name: 'roomReserveList',
    data () {
      return {
        self: this,
        statusOption,

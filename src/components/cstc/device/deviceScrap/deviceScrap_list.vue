<template>
  ...
  <div>
    ...
    <div class="roomButtonList" ref="buttonList">
      <el-button size="small" type="primary" @click="add">新建报废记录</el-button>
      <el-button size="small" type="info" @click="edit">修改报废记录</el-button>
      <el-button size="small" type="danger" @click="remove">删除报废记录</el-button>
    </div>
    <!-- 数据搜索 -->
    ...
    <div id="tableData" ref="tableData">
      <el-table align="center" :context="self" :data="tableData" tooltip-effect="dark" :height="dynamicHt" style="width: 100%"
        ...
        <el-table-column label="报废时间" prop="scrapTime" show-overflow-tooltip>
          <template scope="scope">
            {{ scope.row.scrapTime | formatDate('yyyy-MM-dd HH:mm:ss') }}
          </template>
        </el-table-column>
...
<script>
  ...
  // 模态框
  import add from './deviceScrap_add'; // 新增
  import edit from './deviceScrap_edit'; // 编辑
  import show from './deviceScrap_view'; // 查看

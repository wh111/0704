<template>
  ...
  <div>
    ...
    <Modal close-on-click-modal="false" width="1100" v-model="toChannelModal" title="对话框标题"
      ...
      <to-channel v-if="toChannelModal" @cancel="closeChannel" :url="toChannelUrl"
                  @success="subHandelEvent" :data="convertedData"
                  :format="format" :addData="addData"></to-channel>
      <div slot="footer"></div>
    </Modal>
  </div>
</template>
  ...
  export default {
    ...
    data () {
      return {
        ...
        addData: {trainingId: this.operailityData.trainingId},
    ...
    mounted () {
      ...
      this.http = http + 'static/template/技能培训导入模板.xls';
      this.init();
    },
    methods: {
      /*

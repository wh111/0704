    ...
    <el-form :model="formValidate" ref="formValidate" :rules="rules" class="demo-form-inline" label-width="130px">
      <el-row>
        <el-col :span="9" :offset="2">
          <el-form-item label="判卷任务:" prop="name">
      ...
      <el-row>
        <el-col :span="18" :offset="2">
          <el-form-item label="判卷要求:" prop="requirement">
            <el-input type="textarea" resize="none" :rows="8" v-model="formValidate.requirement"></el-input>
          </el-form-item>
        </el-col>
      </el-row>
      <el-row>
...
<script>
  /*当前组件必要引入*/
  import { llpjamination as rules } from '../rules';
  import api from '../api';
  //当前组件引入全局的util
  let Util = null;
  export default {
    ...
    data () {
      return {
        ...
        llKspjAddTitle: {
          ...
          ajaxParams: {
            jsonString: true,
            url: api.llKspjAdd.path,
            method: api.llKspjAdd.method,
            data: {}
          }
        },
        classnameTitle: {
      ...
      init () {
        ...
        if (this.type == 'edit') {
          ...
          this.editsaveTitle = {
            ...
            ajaxParams: {
              jsonString: true,
              url: api.editllKspj.path + this.operailityData[0].id,
              method: api.editllKspj.method,
              data: {}
            }
          },
            this.listMessTitle = {
              ...
              ajaxParams: {
                url: api.llKspjedit.path + this.operailityData[0].id
              }
            };
      ...
      listenSubEvent (isLoadingFun) {
        ...
        if (isSubmit) {
          ...
          if (formValidate.teachType != '本科教学') {
            formValidate.exameId = null;
            formValidate.className = null;
          }
          ;
          if (this.type == 'add') {
            saveTitle = this.llKspjAddTitle;
          } else {

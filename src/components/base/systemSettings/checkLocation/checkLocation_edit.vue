            ...
            <el-col :span="24">
              <el-form-item label="可打卡对象" prop="name9">
                <el-radio-group v-model="setType">
                  ...
                  <search-select v-if="formValidate.setType=='ROLE'" @setSltOptionValue="setSltRoles"
                                 :selectOptions="selectOptions"></search-select>
                </el-radio-group>
              </el-form-item>
            </el-col>
          </el-row>
    ...
    data() {
      return {
        ...
        editMessTitle: {
          ...
          ajaxError: 'ajaxError',
          ajaxParams: {
            url: '/attendance/clockSet/modify/' + this.operailityData.id,
            method: 'put'
          }
        },
    ...
    methods: {
      ...
      listenSubEvent(isLoadingFun) {
        ...
        if (isSubmit) {
          ...
          data.startToWorkTime = this.conductDate(start, 'HH:mm')+':00';
          data.endOffWorkTime = this.conductDate(end, 'HH:mm')+':00';
      ...
      SuccessGetCurrData(responseData) {
        ...
        this.time={ //因为时间组件已经初始化完成了，时间必须是对象格式的
          start:data.startToWorkTime ? new Date('2010-01-01 ' + data.startToWorkTime) : '',
          end:data.endOffWorkTime ? new Date('2010-01-01 ' + data.endOffWorkTime) : '',
          time:'',
        }

        this.formValidate = data;

        this.setType = this.formValidate.setType;
        this.effectiveType = this.formValidate.effectiveType;
        if (this.setType == "ROLE") {
          for (var i = 0, item; i < data.rolelist.length; i++) {
            item = data.rolelist[i];
            this.selectOptions.value.push(item.roleId);
          }
        }

        let that = this;
        map = new BMap.Map("allmap1");
        let point = new BMap.Point(that.formValidate.longitude, that.formValidate.latitude)// 创建Map实例
        map.centerAndZoom(point, 18);  // 初始化地图,设置中心点坐标和地图级别
        map.setCurrentCity(that.formValidate.placeName);          // 设置地图显示的城市 此项是必须设置的

    ...
    data (){
      return{
        ...
        addMessTitle:{
          ...
          ajaxError:'ajaxError',
          ajaxParams:{
            url:this.url.tubeBedRecordingAdd,
            method:'post',
            data:{},
          }
        },
...
//      保存 改变url
      save(isLoadingFun){
        this.addMessTitle.ajaxParams.url = this.url.tubeBedRecordingAdd;
        this.listenSubEvent(isLoadingFun);
      },

      //保存上报 改变url
      saveReportedEvent(isLoadingFun){
        this.addMessTitle.ajaxParams.url = this.url.tubeBedRecordingAddSubmit;
        this.listenSubEvent(isLoadingFun);
      },
      /*
       * 点击提交按钮 监听是否验证通过

  ...
  <div id="content" ref="content" class="modal">
    <el-form  ref="formValidate" :model="formValidate" :rules="clinicalOperationWriteList" inline label-width="100px">
      <el-row style="margin-bottom:0">
        <!--列表操作按钮-->
        <el-col :span="10" >&nbsp;
          <!--<el-button type="primary" :disabled="disabled.pass" @click="pass">批量通过</el-button>-->
          <!--<el-button type="danger" :disabled="disabled.pass" @click="reject">批量驳回</el-button>-->
        </el-col>
        <!--搜索项-->
      ...
      <div v-show="searchMore" ref="searchMore" align="right">
        ...
        <!--<el-form-item label="状态:"  prop="status">-->
          <!--<el-select v-model="formValidate.status" placeholder="请选择状态">-->
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template scope="scope">
              ...
              <!--<el-button v-if="scope.row.status!='PASS' && scope.row.status != 'REJECT'" size="small" @click="audit(scope.row)">审核</el-button>-->
            </template>
          </el-table-column>
          <el-table-column
    ...
    <Modal
      ...
      <operate v-if="passModal" :type="'pass'" :operate-url="url.clinicalRecordModifySubPass" :methods="'post'"  @cancel="cancel" @operate="subCallback" :operaility-data="operailityData" ></operate>
      <div slot="footer"></div>
    </Modal>
    ...
    <Modal
      ...
      <operate v-if="rejectModal" :type="'reject'" :operate-url="url.clinicalRecordModifySubReject" :methods="'post'"  @cancel="cancel" @operate="subCallback" :operaility-data="operailityData" ></operate>
      <div slot="footer"></div>
    </Modal>
    <!---->
  </div>
...
<script>
  ...
  //引入--新建--组件
  //引入--修改--组件
  import audit from "./extensionRequestAudit_audit.vue";
  //引入--查看--组件
  import show from "./extensionRequestAudit_view.vue";
    ...
    data() {
      return {
        ...
        listMessTitle:{
          ...
          ajaxParams:{
            url:url.delayGivenManageList.path,
            params:{}
          }
        },
        /*--按钮button--*/
    ...
    methods: {
      ...
      handleSelectionChange(val) {
        ...
        for(let i =0 ;i<val.length;i++){
          let item = val[i];
          if(['PASS','REJECT'].includes(item.status)){
            disabled.pass = true;
          }
        }

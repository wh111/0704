<template>
  <div id="content" ref="content" class="modal">
      ...
      <div class="listUpArea-menus">
        ...
        <div class="ivu-row">
          <div class="ivu-col ivu-col-span-24">
            <el-button class="but-col" @click="rotation" :disabled="disabled.noRotation" type="primary">安排轮转</el-button>
            <!--<el-button class="but-col" @click="noRotation" :disabled="disabled.noRotation" type="primary">无需安排轮转</el-button>-->
          </div>
        </div>
      </div>
        ...
        <el-table
          ...
          >
            <template scope="scope">
              ...
              <span v-else-if="scope.row.arrangeDepState=='Y'">已安排</span>
            </template>
          </el-table-column>
    ...
    <Modal
      width="500"
      v-model="noRotationModal"
      title="查看档案管理弹窗"
      class-name="vertical-center-modal"
      :loading="loading">
      <modal-header slot="header" :content="noRotationId"></modal-header>
      <operate v-if="noRotationModal" :type="'noRotation'" :operateData="noRotationData"
               :operate-url="api.modifyDepRotaryState.path" @cancel="cancel" @operate="subCallback"></operate>
      <div slot="footer"></div>
    </Modal>
  </div>
</template>
...
<script>
  /*当前组件必要引入*/
  //引入--微调--组件
  import rotation from "./arrangeRotaryRotation_rotationSet.vue";
  import show from "./arrangeRotaryRotation_view.vue";
  ...
  export default {
    ...
    methods: {
      ...
      handleSelectionChange(val) {
        ...
        val.map(item => {
//           if( ['P','Y'].includes(item.arrangeDepState)){
//             disabled.noRotation = true;
//           }
        })

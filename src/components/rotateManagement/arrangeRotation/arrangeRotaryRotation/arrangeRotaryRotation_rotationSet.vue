...
<template>
  <div>
    <div class="cal-schoolTit">
      {{operailityData["depName"]}}：{{operailityData["ts"]}}{{ operailityData.podClass == 'SXS' ? '周' : '月' }}
    </div>
    <br/>
    <el-table
      v-for="valItem in 1"
              ...
              <el-row>
                <el-col :span="24">
                  <div class="cal-schoolTit">
                    ...
                    <el-input :disabled="formValidate.cType==0" @change.native="watchDeType(formValidate.randomNum)"
                              style="width: 60px;" v-model="formValidate.randomNum" placeholder="请输入内容"></el-input>
                    (<em style="color: #FF0000">该数量指：在所选的院内科室中系统随机抽选几个</em>)
                  </div>
                </el-col>
              </el-row>
              <el-row>
                <el-col :span="24">
                  <div class="cal-schoolTit">
                    <el-radio v-model="formValidate.cType" :label="0">必选科室</el-radio>
                  </div>
                </el-col>
              </el-row>
            </el-radio-group>
      ...
      <el-table-column
        ...
        <template scope="scope">
          ...
          <div v-if="scope.$index==0">
            <el-form :model="{ts:formValidate.depTs}" ref="formValidate" :rules="rules" label-width="0">
              <el-form-item prop="ts">
                <el-input style="width: 60px;" @change="handleRandomInput('depTs')" :disabled="formValidate.cType==0"
                          v-model="formValidate.depTs" placeholder="请输入内容"></el-input>
                {{ operailityData.podClass == 'SXS' ? '周' : '月' }}
              </el-form-item>
            </el-form>
          </div>
          <div v-else>
            <el-form :model="{ts:scope.row.depTs}" ref="formValidate" :rules="rules" label-width="0">
              <el-form-item prop="ts">
                <el-input style="width: 60px;" @change="handleMustInput('depTs')" :disabled="formValidate.cType==1"
                          v-model="scope.row.depTs" placeholder="请输入内容"></el-input>
                {{ operailityData.podClass == 'SXS' ? '周' : '月' }}
              </el-form-item>
            </el-form>
          </div>
    ...
    <div style="text-align: center">
      ...
      <el-button @click="removeDep()" type="danger">移除</el-button>
    </div>
    <div class="cal-schoolTit">待选科室:
    ...
    <div class="sltDepWrapper">
      <ul class="sltDepUl">
        <div v-for="(item,index) in depTreeData" :key="item.id" class="sltDepBox">
          <div class="sltDep">
            <el-checkbox @change="handleCheck(item)" :disabled="item.checked" v-model="item.checked">{{item.name}}
            </el-checkbox>
          </div>
          <div class="sltSubDep" v-if="typeof item.childList!='undefined'"
               v-for="(subItem,subIndex) in item.childList">
            <el-checkbox @change="handleCheck(subItem)" :disabled="subItem.checked" v-model="subItem.checked">
              {{subItem.name}}
            </el-checkbox>
            <li class="sltSubDep" v-if="typeof subItem.childList!='undefined'"
                v-for="(subItems,subIndexs) in subItem.childList">
              <el-checkbox @change="handleCheck(subItems)"
                           :disabled="subItems.checked"
                           v-model="subItems.checked">
                {{subItems.name}}
              </el-checkbox>
            </li>
    ...
    <!-- 模态框 报到 -->
    <Modal :mask-closable="false" v-model="confirmModal" height="200" class-name="vertical-center-modal" :width="450">
      <modal-header slot="header" :content="confirmId"></modal-header>
      <div class="remove">确定之后将无法修改，您确定这样分配吗？</div>
      <el-row>
        <el-col :span="10" :offset="14">
          <load-btn @listenSubEvent="listenSubEvent" :btnData="loadBtn"></load-btn>
          <el-button @click="confirmModal=false" type="danger">取消</el-button>
        </el-col>
      </el-row>
      <div slot="footer"></div>
    </Modal>
...
<script>
  ...
  import { rdyRotationBySXS, rdyRotationByOther } from '../../../zyy/rules';
  //当前组件引入全局的util
  let Util = null;
  export default {
      ...
      return {
        ...
        getDepMessTitle: {
          ...
          ajaxParams: {
            ...
            params: {
              id: this.operailityData.depId,
              depth: this.operailityData.depth,
            }
          }
        },
        ...
        confirmId: {id: 'confirmId', title: '确认'},
      }
    },
    methods: {
      //初始化请求列表数据
      init () {
        this.rules = this.operailityData.podClass == 'SXS' ? rdyRotationBySXS : rdyRotationByOther;
      ...
      initFormateDepTree (data) {
        ...
        //     items.childList.map(ite=>{
        //       this.childList.push(ite)
        //     })
        //   })
        // })
        // console.log( this.childList,2222)
        this.depTreeData = data;
        console.log(this.saveDepTreeData, this.depTreeData, 1121)
      },
      ...
      watchDeType (val) {
        if (isNaN(val)) {
          ...
          if(this.formValidate.depTs&&(this.formValidate.depTs*val>this.operailityData["ts"])){
            this.formValidate.randomNum = 1;
          }
        }
      },
      ...
      confirm () {
        ...
        console.log(totalNum,this.formValidate.depTs,this.formValidate.cType)
        if ((this.formValidate.cType==1&&(Number(this.formValidate.depTs)) != totalNum/this.formValidate.randomNum)||(this.formValidate.cType==0&&(Number(this.formValidate.depTs)) != totalNum)) {
          this.showMess('设置的科室总时间之和必须等于' + this.operailityData['depName'] + ':' + this.operailityData['ts'] + (this.operailityData.podClass == 'SXS' ? '周' : '月'));
          this.confirmModal = false;
          return;
        }
        if (isSubmit) {
      ...
      addDep (id, name, item) {
        ...
        if (this.operailityData.podClass == 'SXS') {
          template.depTs = 1
        } else {
          template.depTs = 0.5
        }
        let totalNum = Number(this.operailityData.ts);
        if (this.tableData1.length != 1 && (Number(this.formValidate.depTs) + template.depTs) > totalNum) {
          item.checked = false;
          this.showMess('设置的科室总时间之和不能大于' + this.operailityData['depName'] + ':' + this.operailityData['ts'] + (this.operailityData.podClass == 'SXS' ? '周' : '月'));
          return;
        }

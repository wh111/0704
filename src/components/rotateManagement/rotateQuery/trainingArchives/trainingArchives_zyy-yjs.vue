      ...
      <div v-if="searchMore" ref="searchMore" style="float: right">
        <el-form-item label="年级" prop="year" >
          <el-select filterable  v-model="formValidate.year" placeholder="请选择">
            <el-option :label="item.label" :value="item.value" :key="index" v-for="(item,index) in yearOpt"></el-option>
          </el-select>
        </el-form-item>
      ...
      >
        <el-table
          ...
          <el-table-column
            ...
            prop="rotaryState"
            label="轮转状态">
            <template scope="scope">{{scope.row.rotaryState | rotaryState }}</template>
          </el-table-column>
...
<script>
  ...
  import url from '../api'
  //引入--查看--组件
  import show from "./trainingArchives_zyy-yjs/trainingArchives_zyy-yjs-view.vue";
  ...
  export default{
    ...
    data() {
      return {
        ...
        formValidate: {
          userName: '',       //大楼名称模糊检索
          userType: this.studentTypes,       //生源类型
          year: '',       //年级
          sortby: '',//排序列
          order: ''     //升序、降序
        },
    ...
    methods: {
      ...
      derive() {
        ...
        this.derivePath =config.urlPrefix+ url.rotaryManulExport.path+item.studentTypes+'-'+item.userId;
        this.operailityData = this.multipleSelection;

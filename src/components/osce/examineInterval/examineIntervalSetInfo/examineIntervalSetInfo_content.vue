  ...
  <div>
    ...
    <el-table align="center" :context="self" :data="tableData" tooltip-effect="dark"
      ...
      <template v-else>
        ...
        <template v-if="stationType === 'SP' || roomInfo.randomType == 'NO'">
      ...
      <el-table-column label="操作" width="180">
        <template scope="scope">
          ...
          <!-- 非理论考试 -->
          <template v-else>
            <el-button size="mini" type="success" style="margin-left:0;margin-right:10px;" @click="openSelectCase(scope.$index)">病例
            </el-button>
            <!--<el-button size="mini" type="warning" style="margin-left:0;margin-right:10px;"-->
            <!--@click="openSelectScore(scope.$index)" :disabled="scope.row.scoreTableList.length==0">评分表-->
            <!--</el-button>-->
            ...
            <template v-if="stationType === 'SP'">
              ...
              <el-button size="mini" type="info" style="margin-left:0;margin-right:10px;" @click="openSelectSPScore(scope.$index)">SP评分表
              </el-button>
            </template>
          </template>
        </template>
    ...
    <Modal :mask-closable="false" v-model="scoreSelectModal" height="200" class-name="vertical-center-modal" :width="840">
      ...
      <score-select v-if="scoreSelectModal" @cancel="cancel" @select="selectScore" :is-only="false"
                    :selectData="tableData[row].scoreTableList"></score-select>
      <div slot="footer"></div>
    </Modal>
    <!-- 模态框 选择SP评分表（select） -->
    ...
    <Modal :mask-closable="false" v-model="editPaperContentModal" height="200" class-name="vertical-center-modal" :width="1000">
      ...
      <edit-paper-content v-if="editPaperContentModal" :paperContent="tableData[row].paperContentList" @cancel="cancel"
                          @save="savePaperContent"></edit-paper-content>
      <div slot="footer"></div>
    </Modal>
    <!-- 模态框 理论考试试题预览 -->
    <Modal :mask-closable="false" v-model="viewPaperContentModal" height="200" class-name="vertical-center-modal" :width="1000">
      ...
      <view-paper-content v-if="viewPaperContentModal" :showData="showPaperContentList" @cancel="cancel"
                          showType="answer"></view-paper-content>
      <div slot="footer"></div>
    </Modal>
  </div>
</template>
  ...
  export default {
    ...
    methods: {
      ...
      next () {
        ...
        // SP考站或者是普通不抽题的考站
        if (this.stationType === 'SP' || this.roomInfo.randomType == 'NO') {
          // 计算时间
          let count = 0;
          for (let i = 0, d = this.tableData, l = d.length; i < l; i++) {
            count += Number(d[i].timeCount);
          }
          this.roomInfo.timeCount = count;
        }
        this.checkTeacherScoreTable();
        this.$emit('goStep', 3);
      },
      /********************** 数据校验 **********************/
      checkData () {
        ...
        for (let i = 0, list = this.tableData, l = list.length; i < l; i++) {
          if (this.stationType == 'THEORY') { // 理论考核
            ...
            // 评分表必须选择
            if (!list[i].scoreTableList.length) {
            ...
            // 时间检测（不抽题）
            if (this.roomInfo.randomType == 'NO' && (isNaN(list[i].timeCount) || !/^\d+(\d?|\.+\d{1})$/.test(list[i].timeCount))) {
              this.errorMess(`第 ${i + 1} 行考核时间必须是有效数字（最多一位小数）！`);
              return false;
            }
          }
        }
      ...
      selectScore (res) {
        let scoreTableList = [];
        let teacher = this.$util._.defaultsDeep([], this.$store.getters['examineInterval/temp/val']('teacher'));
        let scoreTableIds = [];
        (res || []).map(item => {
          scoreTableIds.push(+item.id);
          scoreTableList.push({scoreTableId: item.id, scoreTableName: item.scoreTableName});
        });
        if (teacher.length) { // 如果已经选择了教师
          teacher.map(item => {
            if (item.contentScoreTableObj[this.tableData[this.row]._id]) {
              let scoreTableId = item.contentScoreTableObj[this.tableData[this.row]._id].oldScoreTableId || item.contentScoreTableObj[this.tableData[this.row]._id].scoreTableId;
              // 教师选择了此次选择的评分表中已经不存在的评分表则初始化教师此考核内容的评分表为空
              if (!scoreTableIds.includes(+scoreTableId)) {
                delete item.contentScoreTableObj[this.tableData[this.row]._id];
                console.log('不存在：' + scoreTableId);
              }
            }
          });
          this.$store.commit('examineInterval/temp/initTeacher', teacher);
        }
        console.log(res);
        scoreTableIds = scoreTableIds.join(',');
        this.$store.commit('examineInterval/temp/updateStationContentList', {
          index: this.row, // 需要设置评分表的索引
          value: { // 更新键值对
            scoreTableList, // 教师待选评分表
      ...
      selectCase (res) {
        let scoreRes = [];
//        this.tableData[this.row].scoreTableList = [{scoreTableName: res.scoreTableName, scoreTableId: res.scoreTableId}];
...
//          value.scoreTableName = res.scoreTableName;
          scoreRes = [{scoreTableName: res.scoreTableName, id: res.scoreTableId}];
        }
        scoreRes.length && this.selectScore(scoreRes);
        this.$store.commit('examineInterval/temp/updateStationContentList', {
      ...
      viewPaperContent (index) {
        this.row = index;
        let showPaperContentList = this.$util._.defaultsDeep([], this.tableData[index].paperContentList);

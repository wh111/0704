      ...
      <el-row>
        ...
        <el-col :span="8" :offset="2">
          <el-form-item label="考站类型：" prop="stationType">
            <el-select v-model="info.stationType" filterable placeholder="请选择" @change="changeStationType">
              <template v-for="item in typeOption" v-if="item.value !== ''">
                <template v-if="sceneType === 'FREE'">
                  <el-option v-if="item.value !== 'REST'" :key="item.value" :label="item.label" :value="item.value">
                  </el-option>
                </template>
                <el-option v-else :key="item.value" :label="item.label" :value="item.value">
                </el-option>
              </template>
            </el-select>
        ...
        <el-col :span="8" :offset="2" v-if="!['SP','REST'].includes(info.stationType) && info.randomType !== 'NO'">
        ...
        <el-col :span="8" :offset="2" v-if="!['REST'].includes(info.stationType)">
        ...
        <el-col align="center" style="margin-top:10px;">
          <el-button type="success" @click="next">{{ info.stationType=='REST' ? '完成' : '下一步' }}</el-button>
        </el-col>
      </el-row>
    </el-form>
...
//          },
    ...
    methods: {
      ...
      changeStationType (val) {
        if (val != 'REST') {
          this.$emit('showStep', true);
          return;
        }
        this.info.randomType = 'TEACHER'; // 抽题类型为老师抽题
        this.info.timeCount = 1; // 考试时间为1
        this.room.specialty = 0; // 专业为全部
        this.$emit('showStep', false);
      },
      /**************** 选择房间 ********************/
      selectRoom (val) {
        ...
        // 把选中的房间加入到全部房间集里
        // this.$store.commit('examineInterval/temp/updateAllRoom', {
        //   add: {
        //     roomId: roomVal.id,
        //     roomNum: roomVal.roomNum
        //   },
        //   del: oldRoomId
        // });
    ...
    created () {
      ...
      this.changeStationType(temp.info.stationType);
    }
  };

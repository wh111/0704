<template>
  ...
  <div>
    <!-- 顶部步骤条 -->
    <el-steps :active="active" finish-status="success" :align-center="true" :center="true" space="160px">
    ...
    <!-- 视图 -->
    <info-basic v-if="active === 1" @goStep="goStep" style="margin-top:30px;"></info-basic>
    <info-content v-if="active === 2" @goStep="goStep"></info-content>
    <info-teacher v-if="active === 3" @goStep="goStep"></info-teacher>
    <info-view v-if="active === 4" @goStep="goStep" :basicsTime="basicsTime" :info="info" :room="room" :teacher="teacher" style="margin-top:20px;font-size:14px;">
      <div slot="footer">
        <el-row>

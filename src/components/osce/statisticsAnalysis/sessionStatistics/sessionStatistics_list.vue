...
<template>
  <div id="content" ref="content" class="modal">
      ...
      <el-row>
        <el-col :span="10">
          <el-form-item prop="title" label-width="0px">
            <input class="hidden">
            <el-input v-model="formValidate.sceneName" placeholder="输入场次名称搜索">
              <el-button @click="searchEvent" slot="append" icon="search"></el-button>
            </el-input>
          </el-form-item>
        </el-col>
        <el-col :span="5" :offset="9" align="right">
          <el-button type="primary" @click="add">多场次合并</el-button>
        </el-col>
      </el-row>
    </el-form>
    ...
    <div v-show="isShowList">
      ...
      <div id="myTable" ref="myTable">
        <el-table align="center" :height="dynamicHt" :context="self" :data="tableData" tooltip-effect="dark" highlight-current-row
          ...
          <el-table-column prop="examineeNum" label="学员人数">
          </el-table-column>
          <el-table-column prop="sceneType" label="考核类型">
            <template scope="scope">
    ...
    <entrance-particulars @cancel="goBack" v-if="!isShowList" :operaility-data="operailityData"></entrance-particulars>
    <div>
      <!--新建-->
...
<script>
  ...
  import url from '../app'
  //引入--新建--组件
  import add from './sessionStatistics_add.vue'
  ...
  export default {
    ...
    methods: {
      ...
      changePage (n) {
        ...
        this.forTheMoment=this.forTheMoment.concat( this.multipleSelection )
        this.setTableData();
      },
      //设置表格及分页的位置
      setTableDynHeight() {
      ...
      entrance(data) {
        ...
        this.operailityData.sceneId = data.id;
      ...
      /*--点击--添加--按钮--*/
      add() {
        console.log(this.forTheMoment)
        console.log(this.multipleSelection)
        if (this.forTheMoment.length+this.multipleSelection.length < 2) {
          this.showMess("至少选择两条数据!")
          return;
        }
        this.operailityData = this.forTheMoment.concat(this.multipleSelection);
        this.openModel("add");
      },
      /*--点击--修改--按钮--*/
      edit(data) {

  ...
  <div class="modalContent" ref="content">
    ...
    <div class="contentMain">
      <el-tabs v-model="showName">
        <el-tab-pane label="指标明细" name="info"></el-tab-pane>
        <el-tab-pane label="图表分析" name="chart" v-if="!!stationId"></el-tab-pane>
      </el-tabs>
      <template v-if="showName=='info'">
        <el-tabs v-model="activeName" @tab-click="handleClick">
          <el-tab-pane v-for="(item,index) in contentList" :key="index" :label="item.contentName"
                       :name="index+''"></el-tab-pane>
        </el-tabs>
        <div>
          <el-select v-if="operailityData.stationType !== 'THEORY'" style="margin-left: 20px;margin-bottom: 10px" placeholder="请选择" v-model="scoreTableId"
                     @change="setScoreData">
            <el-option v-for="(item,index) in listOption" :key="item.scoreTableId" :label="item.scoreTableName"
                       :value="item.scoreTableId"></el-option>
          </el-select>
          <!--表格数据-->
          <div id="myTable" ref="myTable">
            <el-table v-if="operailityData.stationType === 'THEORY'" align="center" :height="300"
            ...
            <template v-else>
              <el-table align="center" :height="300" :data="contentDataList.detailsList" tooltip-effect="dark"
                        highlight-current-row style="width: 100%;height: 100%">
                <el-table-column align="left" label="编号" type="index" width="90">
                </el-table-column>
                <el-table-column prop="content" label="评分项目" show-overflow-tooltip>
                </el-table-column>
                <el-table-column prop="score" label="分值" align="right" width="100">
                </el-table-column>
                <el-table-column prop="avgMark" label="平均分" align="right" width="100">
                </el-table-column>
            ...
            <el-table align="center" :height="200" :data="markList" tooltip-effect="dark" highlight-current-row
              ...
              <el-table-column prop="teacherName" label="考官姓名" align="center" v-if="operailityData.stationType !== 'THEORY'">
              </el-table-column>
              <el-table-column prop="totalMark" label="得分" align="right">
              </el-table-column>
            </el-table>
          </div>
        </div>
      </template>
      <sub-chart v-else :scoreTableId="scoreTableId" :stationId="stationId" :url="url"></sub-chart>
    </div>
    <!--查看弹窗-->
...
<script>
  //当前组件引入全局的util
  import subChart from './substation_chart';
  ...
  export default {
    ...
    methods: {
      ...
      init () {
        ...
        //ajax请求参数设置
        this.setTableData();
        console.log(this.url.contentGet);
      },
      //设置表格及分页的位置
      setTableDynHeight () {
      ...
      updateListData (responseData) {
        ...
        if (data.contentList) {
          ...
          if (this.operailityData.stationType === 'THEORY') {
            this.setScoreData(data.contentList[0]);
          } else {
            this.listOption = data.contentList[0].scoreTableList;
            this.scoreTableId = data.contentList[0].scoreTableList[0].scoreTableId;
...
//        this.scoreTableDetailsList = data.detatilsList;
      ...
      // 获取试卷内容（理论考核专用）
      setScoreDataByPaper (scoreValidate) {
        let opt = {
          ajaxSuccess: res => {
          ...
          ajaxParams: {
            ...
            params: {
              contentId: scoreValidate.contentId, // 考核内容id
              stationId: this.operailityData.stationId // 考站id
            }
          }
        };
        this.ajax(opt);
      ...
      setScoreData (scoreValidate) {
        ...
        if (this.operailityData.stationType === 'THEORY') { // 理论考核
          ...
          this.setScoreDataByPaper(scoreValidate);
          return;
        }
      ...
      handleClick (tab) {
        ...
        if (this.operailityData.stationType === 'THEORY') {
          ...
          let data = this.data;
          console.log(data);
          this.listOption = data.contentList[index].scoreTableList;
          this.scoreTableId = data.contentList[index].scoreTableList[0].scoreTableId;
//          this.stationId=data.contentList[index].scoreTableList[0].stationId
          this.setScoreData();
        }

    ...
    methods: {
      ...
      confirm(isLoadingFun){
       ...
       let  queryData = this.conductQueryData(this.body==0?this.tempArr:this.body)
        ...
        let that = this;
        queryData.map(item =>{
          if(item.hospitalTime.indexOf(':') == -1){
            that.conductDate(item.hospitalTime,'yyyy-MM-dd HH:mm:ss')
          }
        })
        //处理服务数据
      ...
      conductQueryData(data){
        ...
        let index = 0;
        let tempArr  =[];
        for(let i=0;i<queryData.length;i++) {  //把数字变成英文名字,并符合接口数据
...
//        }
        ...
        for(let i=0;i<data.length;i++){
          ...
          if(format[data[i]]) formats.push(format[data[i]]);
        }

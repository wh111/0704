...
<template>
  <div>
    ...
    <el-row class="but-space">
      ...
      <el-col :span="16">
        <p>上传填好的病例信息表 ( 仅支持.xls/.xlsx格式)</p></br>
    ...
    <Modal
      ...
      <!--<to-channel v-if="toChannelModal" @cancel="closeChannel" :date="date" :id="id" :treeRoad="treeRoad"-->
      <!--:url="url.save.path" @success="subHandelEvent" :data="convertedData" :format="format"></to-channel>-->
      <to-channel v-if="toChannelModal" @cancel="closeChannel" :id="id" :treeRoad="treeRoad" :url="url.save.path" @success="subHandelEvent"
                  :data="convertedData" :commitBefFun="commitBefFun" :format="format"></to-channel>
      <div slot="footer"></div>
    </Modal>
  </div>
</template>
  ...
  export default {
    ...
    data () {
      return {
        ...
        toChannelId: {id: 'toChannelId', title: '病例信息表'},
        convertedData: {},
//        ch2en: {
//          sex: {
//            '男': 'BOY',
...
//        },
    ...
    methods: {
      ...
      init () {
        ...
        this.http = http + 'static/template/病例导入模板.xls';
      },
      /*
       * 点击提交按钮 监听是否提交数据
       ...
       * */
      success () {
        this.$emit('cases', 'cases', `导入${this.totalNumber}条数据`);
      },

      cancel () {
        this.$emit('cancel', 'cases');
      },
      commitBefFun (arr) {
        arr.map(item => item.questionList = [{question: item.question}]);
        return arr;
      },
      /*
       * 获取表单数据

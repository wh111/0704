...
<script>
  export default {
     ...
     * height :  最小高度
     * */
    props: ['format', 'height', 'url', 'data', 'extra', 'id', 'treeRoad', 'date', 'commitBefFun'],
    data () {
    ...
    methods: {
      ...
      confirm (isLoadingFun) {
        ...
        if (this.commitBefFun) {
          queryData = this.commitBefFun(queryData);
        }
        console.log(this.totalScore);
        if (!this.noIsPoints) {
          this.errorMess(`扣分项必须填写！`);
          return false;
        }
        // 验证分数和评分指标类型
        for (let i = 0; i < queryData.length; i++) {
          if (queryData[i].totalScore != this.totalScore[i]) {
            this.errorMess(`${queryData[i].scoreTableName} 的总分不是各项标准分（非扣分项）之和，请重新计算总分！`);
            return false;
          }
          if (!queryData[i].scoreType) {
            this.errorMess(`${queryData[i].scoreTableName} 的评分指标类型必须填写！`);
            return false;
          }
          for (let j = 0; j < queryData[i].detailsList.length; j++) {
            let dItem = queryData[i].detailsList[j];
            if (dItem.classifyDescribe !== '' || dItem.classifyScore !== '') {
              let classifyDescribe = (dItem.classifyDescribe || '').split(';');
              let classifyScore = (dItem.classifyScore || '').split(';');
              let noNum = classifyScore.some(num => isNaN(num));
              if (classifyDescribe.length != classifyScore.length) {
                this.errorMess(`${queryData[i].scoreTableName} 的第 ${j + 1} 行评分指标等级描述与对应分值不匹配！`);
                return false;
              }
              if (noNum) {
                this.errorMess(`${queryData[i].scoreTableName} 的第 ${j + 1} 行对应分值填写错误！`);
                return false;
              }
            }
          }
...
//        }
        ...
        let totalScore = [];
        let noIsPoints = false;
        for (let i = 0; i < queryData.length; i++) {
          if (queryData[i].scoreTableName) {
            index++;
            tempArr.push({
              scoreType: queryData[i].scoreType,
          ...
          if (!noIsPoints) {
            noIsPoints = !!queryData[i].isPoints;
          }
          // 累计总分的时候排除扣分项
          totalScore[index] += (queryData[i].isPoints == '否' ? Number(queryData[i].score) : 0);
        }
        this.totalScore = totalScore;
        this.noIsPoints = noIsPoints;
        return tempArr;
      },

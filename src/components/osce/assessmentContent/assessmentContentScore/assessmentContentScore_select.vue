    ...
    <div slot="right" id="content" ref="content" style="padding:0;">
      ...
      <div id="tableData" ref="tableData">
        <el-table align="center" :height="dynamicHt" :context="self" :data="tableData" tooltip-effect="dark" class="tableShowMoreInfo"
          ...
          <el-table-column label="总分" prop="totalScore" show-overflow-tooltip width="80"></el-table-column>
        </el-table>
      </div>
      <!-- 分页 -->
...
<script>
  ...
  import scoreApi from './api' // 评分表api
  ...
  export default {
    props: {
      ...
      selectData: { // 已经选择了的数据 // todo 未做相关回显处理
        type: Array,
        default: () => [], // {_id:'选择的id',....其他数据会原样返回}
      },
    },
    data () {
      return {
    ...
    methods: {
      ...
      init () {
        ...
        this.queryQptions = {
          url: scoreApi.list.path,
          method: scoreApi.list.method,
          params: {
            curPage: 1,
      ...
      // 选择评分表
      listenSubEvent () {
        if (this.isSelected(this.isOnly)) {
          this.$emit('select', this.isOnly ? this.multipleSelection[0] : this.multipleSelection)
          this.cancel()
        }
      },
      //---------------------------------- 左侧菜单树 ------------------------------------------//
      ...
      isSelected (isOnly) {
        ...
        let flag = true
        if (len == 0) {
          this.showMess('请选择一个分表!')
          flag = false
        }
        if (len > 1 && isOnly) {
          this.showMess('只能选择一个评分表!')
          flag = false
        }
        return flag
      },

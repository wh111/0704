...
<template>
  <div>
    ...
    <el-row class="but-space">
      ...
      <el-col :span="16">
        <p>上传填好的评分信息表 ( 仅支持.xls/.xlsx格式)</p></br>
    ...
    <Modal
      ...
      <to-channel v-if="toChannelModal" :id="id" :commitBefFun="commitBefFun" :treeRoad="treeRoad" @cancel="closeChannel" :url="url.save.path" @success="subHandelEvent"
...
<script>
  import toChannel from './toChannel_baseOfScore.vue';
  //当前组件引入全局的util
  let Util = null;
  export default {
    //props接收父组件传递过来的数据
    ...
    data () {
      return {
        ...
        toChannelId: {id: 'toChannelId', title: '评分信息表'},
        convertedData: {},
        format: {
          '评分表名称': 'scoreTableName',
          '总分数': 'totalScore',
          '评分指标类型': 'scoreType',
          '评分标准及要求': 'content',
          '标准分': 'score',
          '评分指标等级描述': 'classifyDescribe',
          '对应分值': 'classifyScore',
          '评分项顺序': 'detailOrder',
          '项目': 'classify',
          '描述': 'remark',
...
//        },
    ...
    methods: {
      ...
      init () {
        ...
        this.http = http + 'static/template/评分表导入模板.xls';
      },

      commitBefFun (data) {
        data.map(item => {
          item.scoreType = this.convertType(item.scoreType);
          item.detailsList.map(temp => {
            temp.isPoints = this.convertPoints(temp.isPoints);
          });
        });
        return data;
      },

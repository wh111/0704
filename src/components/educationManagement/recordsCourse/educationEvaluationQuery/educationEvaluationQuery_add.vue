      ...
      <el-col :span="24" align="right">
        ...
        <el-form :inline="true" :model="searchObj">
          <el-form-item  label="姓名:" prop="userName">
            <el-input   v-model="searchObj.userName" @change="search" placeholder="输入姓名搜索">
            </el-input>
          </el-form-item>
...
//              setTimeout(()=>this.setTableData(),100)
      ...
      // 选择科室
      changeQTBM(index){
        this.searchObj.depIds = this.queryTreeByManager[index].id;
        this.search();
//        this.searchObj.depName = this.queryTreeByManager[index].name;
      ...
      tableAddData(arr){
        let val = ['lateArrivalTimes','leaveEarlyTimes','askLeaveDays','minerDays','otherBreachDiscipline','classRecords','unqualifiedPrescription',
          'seriousMedicalErrors','medicalAccident','redEnvelopeCount','redEnvelopeAmount','rebateCount','rebateAmount','patientComplaints'
        ];
        arr.map(item=>{
          // 默认值
          item.reviewTime = this.conductDate(this.selectObj.reviewTime, 'yyyy-MM');

  ...
  <div id="content" ref="content" class="modal">
    ...
    <div>
      ...
      <Modal :mask-closable="false" close-on-click-modal="false" height="200" v-model="toChannelModal" class-name="vertical-center-modal" :width="800">
        ...
        <toChannel v-if="toChannelModal" :deptId="deptId" @toChannel="toChannelSuccess" @cancel="cancel" :operaility-data="operailityData"></toChannel>
        <div slot="footer"></div>
      </Modal>
      <!--导出弹窗-->
      ...
      <Modal :mask-closable="false" v-model="depMoveModal" height="200" class-name="vertical-center-modal" :width="960">
        ...
        <dep-move v-if="depMoveModal" @cancel="cancel" :parentTreeDefaults="treeDefaults" @depMove="subCallback"
...
<script>
  /*当前组件必要引入*/
  //引入--修改--组件
  import edit from "./jxsPersonnelManagement_edit";
  //引入--查看--组件
  import show from "./jxsPersonnelManagement_view.vue";
  //引入--添加--组件
  import add from "./jxsPersonnelManagement_add";
  //引入--导入--组件
  import toChannel from "./jxsPersonnelManagement_toChannel.vue";
  //引入--短信通知--组件
  ...
  let store = null;
  export default {
    props: ['deptId', 'deptName', 'fromWhereTree', 'admin', 'treeDefaults'],
    data() {
      return {
        sexOption,
        auditStatusOption,

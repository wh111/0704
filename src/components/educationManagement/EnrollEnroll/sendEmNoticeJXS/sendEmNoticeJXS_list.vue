  ...
  <div id="content" ref="content">
    ...
    <el-form :inline="true" :model="formInline" class="demo-form-inline">
      ...
      <el-form-item label="状态">
        <el-select v-model="formInline.noticeSend" placeholder="请选择">
    ...
    <div>
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template scope="scope">
              <el-button size="small" type="info" @click="show(scope.row,'show')" :disabled="scope.row.noticeSend==0">查看</el-button>
              <el-button size="small" type="info" @click="show(scope.row,'change')" :disabled="scope.row.noticeSend==1">修改</el-button>
          ...
          >
            <template scope="scope">
              {{ scope.row.noticeSend | noticeSend}}
            </template>
          </el-table-column>
        </el-table>
      </div>
      ...
      <!-- 模态框  发送进修通知 -->
      <Modal :mask-closable="false" v-model="sendModal" height="200" class-name="vertical-center-modal" :width="500">
        <modal-header slot="header" :content="button.sendId"></modal-header>
        <div>
          <div class="modalTips">确认发送选中的进修通知？
          </div>
          <el-row>
            <el-col :span="10" :offset="14">
              <el-button @click="sendCall" type="primary" >确定</el-button>
              <el-button class="but-col" @click="cancel('send')">取消</el-button>
            </el-col>
          </el-row>
        </div>
        <div slot="footer"></div>
      ...
      <Modal :mask-closable="false" v-model="showModal" height="200" title="对话框标题" class-name="vertical-center-modal"
        ...
        <show v-if="showModal" @cancel="cancel" :operaility-data="operailityData" :type="type"></show>
        <div slot="footer"></div>
      </Modal>
...
<script>
  ...
  import show from './showEmNoticeJXS';
...
//        this.getQueryTreeByManager();
      ...
      sendCall() {
        ...
        this.multipleSelection.map((item)=>{
          temp.push(item.subjectsId)
        })
        let opt = {
          ajaxSuccess: res =>{this.successMess('发送成功');this.cancel('send');this.setTableData();} ,
          ajaxParams: {
            url: api.batchSendNotice.path+temp.join(','),
            method: api.batchSendNotice.method,
          }
        };
        this.ajax(opt)
      },
...
//            url:"/studysubject/list",
      ...
      // 查看
      show(row,type) {
        this.button.showId.title=(type=='change'?'修改': '查看');
        this.type=type;

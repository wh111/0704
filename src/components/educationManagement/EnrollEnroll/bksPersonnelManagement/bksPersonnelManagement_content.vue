    ...
    <div>
      ...
      <Modal :mask-closable="false" v-model="depMoveModal" height="200" class-name="vertical-center-modal" :width="960">
        ...
        <dep-move v-if="depMoveModal" @cancel="cancel" @depMove="subCallback"
                  :operailityData="operailityData" :parentTreeDefaults="treeDefaults"></dep-move>
        <div slot="footer"></div>
      </Modal>
      <!---->
    </div>
...
<script>
  /*当前组件必要引入*/
  //引入--修改--组件
  import edit from "./bksPersonnelManagement_edit";
  //引入--查看--组件
  import show from "./bksPersonnelManagement_view.vue";
  //引入--添加--组件
  import add from "./bksPersonnelManagement_add";
  //引入--导入--组件
  import toChannel from "./bksPersonnelManagement_toChannel.vue";
  //引入--短信通知--组件
  ...
  export default {
    //isClassNode  是否是班级节点
    props: ['deptId', 'deptName', 'fromWhereTree', 'admin', 'isClassNode', 'nodeNumber', 'treeDefaults'],
    data() {
      return {
        sexOption,
        auditStatusOption,
    ...
    methods: {
      ...
      /**能否添加节点*/
      canOperation() {
        if (!this.isClassNode) this.errorMess('请选择班级节点');
        return this.isClassNode
      },

      canToChannel() {
        if (this.nodeNumber < 4) {
          this.errorMess('请选择其子节点');
          return false;
        } else {
          return true;
        }

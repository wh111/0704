  ...
  <div id="content" ref="content">
    ...
    <el-form :inline="true" :model="formInline" class="demo-form-inline">
      ...
      <el-form-item label="状态">
        <el-select v-model="formInline.confirmState" placeholder="请选择">
      ...
      <el-form-item>
        ...
        <el-button type="success" @click="putConfirm">批量发送</el-button>
      </el-form-item>
    </el-form>
    ...
    <div>
      ...
      >
        <el-table
          ...
          >
            <template scope="scope">
              {{ scope.row.confirmState==1?'已发送':'未发送' }}
            </template>
          </el-table-column>
        </el-table>
      </div>
      ...
      putConfirm(){
        if(this.multipleSelection.length==0){
          ...
          this.multipleSelection.map((item)=>{
            m.push(item.subjectsId)
          })
        }

        this.ajax({
          ajaxParams: {
            url: api.modifyState.path + m.join(','),
            method:'put'
          },
          ajaxSuccess: () => {

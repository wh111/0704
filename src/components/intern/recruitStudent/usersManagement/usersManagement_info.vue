...
<template>
  <div :class="{'userInfoAllShow': !showTab }">
    ...
    <basic class="userInfoItem" :dataId="operailityData.id" v-show="!showTab || activeName==='basic'" ref="basic"
           @save="saveCall" :addRules="basicAddRules" :mustUploadHP="mustUploadHP" :formWhere="formWhere"></basic>
    <education class="userInfoItem" :dataId="operailityData.id" v-show="!showTab || activeName==='education'"
               ref="education" @save="saveCall"></education>
    ...
    <credentials class="userInfoItem" :dataId="operailityData.id" v-show="!showTab || activeName==='credentials'"
                 ref="credentials" @save="saveCall"></credentials>
    ...
    <study class="userInfoItem" :dataId="operailityData.id" v-show="!showTab || activeName==='study'" ref="study"
           @save="saveCall"></study>
    <train class="userInfoItem" :dataId="operailityData.id" v-show="!showTab || activeName==='train'" ref="train"
           @save="saveCall"></train>
    <awards class="userInfoItem" :dataId="operailityData.id" v-show="!showTab || activeName==='awards'" ref="awards"
            @save="saveCall"></awards>
    ...
    <papers class="userInfoItem" :dataId="operailityData.id" v-show="!showTab || activeName==='papers'" ref="papers"
            @save="saveCall"></papers>
    ...
    <div align="center">
      ...
      <template v-if="userInfo">
        <template
          v-if="['NOT_SUBMIT','AUDIT_FAILURE'].includes(userInfo.auditStatus)||userInfo.auditStatus=='AUDIT_SUCCESS' && userInfo.archivesAuditStatus!='NOT_AUDIT'">
          <load-btn @listenSubEvent="listenSubEvent" :btnData="loadBtn"></load-btn>
          <load-btn @listenSbEvent="listenSbEvent" v-if="userInfo.auditStatus!='AUDIT_SUCCESS'"
                    :btnData="sbBtn"></load-btn>
        </template>
        <span v-else style="margin-right: 10px;color: #FF4949;">您的档案信息正在审核中……</span>
      </template>
      <load-btn v-if="userInfo == undefined" @listenSubEvent="listenSubEvent" :btnData="loadBtn"></load-btn>
...
<script>
  /*引入查看的子组件*/
  //  引入--基本情况--组件
  import basic from '../../../common/archivesManagement/archivesManagement_basic/s_sxs/input.vue';
  //  引入--学历信息--组件
  ...
  export default {
    // mustUploadHP 必须上传头像
    props: ['operailityData', 'userInfo', 'mustUploadHP', 'formWhere'],
    data() {
      return {
        basicAddRules,
        showTab: true,
        ...
        sbBtn: {title: '上报', callParEvent: 'listenSbEvent'},
        //上报审核
        auditMessTitle: {
          type: 'edit',
          successTitle: '上报成功!',
...
//        allTab:['basic','education','workExperience','professionalSkill','credentials','teachingExperience','study','train','awards','scientific','papers'],
        allTab: ['basic', 'education', 'credentials', 'study', 'train', 'awards', 'papers'],
        saveTab: [],
      };
    },
    methods: {

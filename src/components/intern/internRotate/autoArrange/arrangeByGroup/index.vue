...
<template>
  <div>
    <el-steps style="margin: 0 auto; width: 98%;" :space="240" :active="active" finish-status="success">
      <el-step title="第一步：选择分组"></el-step>
      <el-step title="第二步：设置轮转开始时间及要求"></el-step>
      <el-step title="第三步：确认"></el-step>
      <el-step title="第四步：预览"></el-step>
    ...
    <div v-show="active==0" style="margin: 20px;">
      ...
      <!--<el-row style="margin-top: 10px;">
        ...
        <el-col :span="14">
          ...
          <span v-if="getDgBySpecialty.length>0&&sltedDg==''" style="color: #FF0000">&nbsp;&nbsp;&nbsp;&nbsp;请选择对应的大纲!</span>
        </el-col>
      </el-row>-->
    ...
    <div v-if="active==1" style="margin: 20px;">
      ...
      <el-row style="margin-top: 10px;">
        <el-col :span="10">
          <div class="cal-schoolTit" style="text-align: right;">科室轮转要求：</div>
        </el-col>
        <el-col :span="10">
          <el-select
            v-model="sltedDepReq"
          ...
          <span v-if="depReqData.length>0&&sltedDepReq==''"
                style="color: #FF0000">&nbsp;&nbsp;&nbsp;&nbsp;请选择对应科室轮转要求!</span>
        </el-col>
      </el-row>
      <br>
      <el-row>
        <el-col :span="10">
          <div class="cal-schoolTit" style="text-align: right;">轮转开始时间：</div>
        </el-col>
        <el-col :span="10">
          <el-date-picker
    ...
    <!--- 第三步：确认 --->
    <div v-if="active==2" style="margin: 10px 20px;">
      <!--  <el-row>
          <el-col :span="20">
            <div class="cal-schoolTit">学校：{{saveSchoolData.schoolName || ""}}</div>
          </el-col>
        </el-row>-->
      <el-row>
        <el-col :span="20">
          <div class="cal-schoolTit">轮转开始时间：{{startRotateTime || ""}}</div>
        </el-col>
      </el-row>
      <el-row :gutter="10" class="myCard"
              style=" max-height: 420px;overflow: auto;box-shadow: 1px 0px 2px rgba(0, 0, 0, 0.32) ">
        <el-col style="margin: 10px 0; overflow: hidden; float: left" :span="12" v-for="(o, index) in groupData"
                :key="o">
          <el-card :body-style="{ padding: '0px', margin:'0 auto' }">
            <div style="padding: 14px;">
              <div class="el-form selectUserBox">
                <fieldset style="min-height:90px;">
                  <legend style="font-size:16px">&nbsp;&nbsp;{{o.groupName}}：{{o.groupNoOrderName}}  组员名单&nbsp;&nbsp;
                  </legend>
                  <div style="height:40px;overflow:auto;" v-if="o.groupUserNames.length>0">
                    <el-tag style="margin: 2px;" v-for="(item,idx) in o.groupUserNames" :key="idx" type="success">
                      {{item}}
                    </el-tag>
                  </div>
                  <p v-else>此组还没有添加组员!</p>
                </fieldset>
              </div>
            </div>
          </el-card>
        </el-col>
        <el-col style="margin: 10px 0; overflow: hidden; float: left" :span="24" v-if="groupData.length==0">
          <p style="text-align: center;color: #f00;font-size: 14px;padding: 20px;">当前学校对应专业的科室要求没有组可以安排，请重新选择!</p>
        </el-col>
      </el-row>
...
<script>
  /*当前组件必要引入*/
  import first from './arrangeByGroup_first.vue'
  ...
  export default {
    ...
    data () {
      return {
        ...
        //第三步获取组的信息
        getGroupPeopleTitle: {
          ajaxSuccess: 'getGroupPeopleData',
          ajaxParams: {
            url: api.getGroupPeopleDetails.path,
            params: {}
          }
        },
    ...
    methods: {
      ...
      //设置第三步的组信息
      getGroupPeopleData (responseData) {
        let data = Util._.defaultsDeep([], responseData.data)
        this.groupData = [];
//        this.postData.groups = [];
        for (var i = 0, item; i < data.length; i++) {
          item = data[i];
          /*this.postData.groups.push({
          ...
          if (item['groupUserNames'].indexOf(',') > -1) {
            ...
            for (var k = 0, subItemName, subItemId; k < item["groupUserNames"].split(",").length; k++) {
              ...
              namesArr.push(subItemName);
              idsArr.push(subItemId);
            }
      ...
      next () {
        ...
        if (active == 0) {
          let firstData = this.$refs.first.parentGetData();
          if (!firstData) return;//如果不存在则有错
          this.postData = Object.assign(this.postData, firstData)
          this.active++
          return;
        }
        ...
        if (this.startRotateTime == '' && this.active == 1) {
        ...
        if (this.active == 1) {
          ...
          /*this.getGroupPeopleTitle.ajaxParams.params = {
            schoolId: this.saveSchoolData['schoolId'],
            //specialty:this.sltedSpecialty,
            outlineId: this.sltedDg,
          }
          this.ajax(this.getGroupPeopleTitle);
          return;*/
//          console.log(this.postData.groups);
          this.getGroupPeopleData({data: this.postData.groups})
          return;
        }

        if (this.groupData.length == 0 && this.active == 2) {
          this.errorMess('当前学校对应专业的科室要求没有组可以安排!');
          return;
        }

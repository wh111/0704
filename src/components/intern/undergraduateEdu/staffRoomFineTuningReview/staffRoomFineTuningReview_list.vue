...
<template>
  <div id="content" ref="content" class="modal">
          ...
          <el-table-column
            ...
            <template scope="scope">
              ...
              <el-button size="small"
                         :disabled="![2,4].includes(scope.row.state)"
                         @click="audit(scope.row)">审核
              </el-button>
              <el-button size="small"
                         :disabled="![2,4].includes(scope.row.state)"
                         @click="send(scope.row)">发送
              </el-button>
    ...
    <Modal
      ...
      <thirdDepList v-if="showModal" @cancel="cancel" @show="subCallback" :operaility-data="operailityData"
                    :typeView="'show'"></thirdDepList>
      <div slot="footer"></div>
    </Modal>
    <!--审核-->
    <Modal
      ...
      <thirdDepList v-if="auditModal" :url="url" @audit="subCallback" @cancel="cancel"
                    :operaility-data="operailityData"></thirdDepList>
      <div slot="footer"></div>
    </Modal>
    <!--发送-->
    <!--发布周历-->
    <Modal
      ...
      :loading="loading"
      :width="500">
      <modal-header slot="header" :content="sendId"></modal-header>
      <operate v-if="sendModal" :type="'send'" title="发送" :messTitle="{title:'发送到教育处？'}" :operate-data="sendData" @operate="subCallback"
...
<script>
  ...
  import url from '../api'
  //引入--查看--组件
  import thirdDepList from './staffRoomFineTuningReview_thirdDepList.vue'
  //当前组件引入全局的util
  let Util = null;
  export default {
    props: {
    ...
    data() {
      return {
        ...
        sendData: {
          url: url.researchRoomReview.path,
          data: {
            type: 'jys',
          }
        },
    ...
    methods: {
      ...
      /*--点击--导出--按钮--*/
      deriveCourse({weekSetId, courseTeachingResearchRoom}) {
        this.deriveCourseUrl = '/api/' + this.url.exportCourseTable.path + '?' + Util.serializeParams({
          weekSetId: weekSetId,
          courseTeachingRooms: courseTeachingResearchRoom
        });
       ...
       * */
      send(data) {
        this.sendData.data = Object.assign(this.sendData.data, {
          weekSetId: data.weekSetId,
          outlineId: data.outlineId,
          courseTeachingResearchRoom: data.courseTeachingResearchRoom,
          state: '3',
        })
        this.openModel('send');
      },

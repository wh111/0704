...
<template>
  <div>
    ...
    <div class="newCalendar calendareManagement" v-if="stepMaker=='third'">
      ...
      <auditList :editOperailityData="editOperailityData" v-show="activeName==='list'"></auditList>
      <br>
...
<script>
  ...
  import setCurriculum from '../depSyllabusManagement/setCurriculum.vue.vue';

  import auditList from './staffRoomFineTuningReview_audtiList.vue';
  //引入日历相关的配置
    ...
    data() {
      return {
        ...
        getMonthCourseSet: {
          ...
          ajaxParams: {
            url: api.getCourseSet.path,
            method: api.getCourseSet.method,
          }
        },
        ...
        let holidayListTitle = {
          ajaxSuccess: (res) => {
            ...
            this.getMonthCourseSet.ajaxParams.params = {
              ...
              depId: this.editOperailityData.courseTeachingResearchRoomThird,
              type: 'three',
            }
...
//                title: currFormate[contentArr[i]["timeType"]]+num+":"+contentArr[i]["depName"]+"   "+contentArr[i]["timeInfo"],
                title: `${num}、${contentArr[i]['outlineName'] || '—'}  课程：${contentArr[i]['courseName'] || '—'} 科室：${contentArr[i]['courseTeachingResearchRoomThirdName'] || '—'}    内容:${contentArr[i]['courseContent'] || '—'}   教师：${contentArr[i].teacherNames || '—'}  教学方法：${contentArr[i].courseMethod || '—'} 类型：${contentArr[i].courseType || '—'} 时间：${contentArr[i]['timeInfoBegin'] || ''}-${contentArr[i]['timeInfoEnd'] || '—'}`,
                start: k,
                end: k,

  ...
  <div id="content" ref="content" class="modal">
      ...
      <el-table
        ...
        <el-table-column
          ...
          <template scope="scope">
            ...
            <el-button
              :disabled="![1,4].includes(scope.row.state)"
              size="small"
              type="primary"
              @click="edit(scope.$index, scope.row)">编辑
            </el-button>
            <el-button v-if="[1,4].includes(scope.row.state)" size="small" @click="publish(scope.row)"
                       type="success">发布
            </el-button>
          </template>
        </el-table-column>
    ...
    <div style="margin: 10px;">
      ...
      <Modal
        ...
        <edit v-if="showModal" type="show" :edit-operaility-data="editOperailityData"></edit>
        <div slot="footer"></div>
      </Modal>
      <!--下发周历-->
      <Modal
        ...
        <operate v-if="modifyStateModal" title="发布" :type="'modifyState'" :messTitle="{title:`发送到${modifyStateIssuedData.data.name}？`}"
                 :operate-data="modifyStateIssuedData" @operate="subCallback" @cancel="cancel"
                 :operaility-data="editOperailityData"></operate>
        <div slot="footer"></div>
      </Modal>
    </div>
  </div>
...
<script>
  /*当前组件必要引入*/
  //引入--修改--组件
  import edit from './depSyllabusManagement_edit.vue';
  //引入当前组件字典api
    ...
    methods: {
      ...
      publish (rowData) {
        this.modifyStateIssuedData.data = {
          ...
          name:rowData.courseTeachingResearchRoomName,
        }

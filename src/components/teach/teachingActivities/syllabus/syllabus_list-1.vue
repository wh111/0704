...
<script>
  import listHeaders from './listHeaders-1.vue';
  import url from '../app';
  let Util = null;
  export default {
      props:['userType'],
          ...
          if(data[0]&& data[0].activityTime.replace(/(^\s*)|(\s*$)/g, "")== this.getDate(ascending).replace(/(^\s*)|(\s*$)/g, "")){
            ...
            for(let i=0;i<recordTimeIds.length;i++){
              ...
              if(tableData[index][key]){
                tableData[index][key] =  tableData[index][key] +';'+ cell.activityName;  //为tableData添加内容
              }else {
                tableData[index][key] = cell.activityName;  //为tableData添加内容
              }
             if(tableData[index][key+'-id']){
               let ids = tableData[index][key+'-id'].split(',');
               ids.push(cell.activityId);
               tableData[index][key+'-id'] = ids.join(',');
             }else {

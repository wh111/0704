<!--菜单管理-->
<template>
  <div class="container" :class="defaults.cls">
    <div v-if="defaults.isShowSearch" class="treeSearchBox">
      <el-input placeholder="输入关键字进行过滤" v-model="filterText">
      </el-input>
    </div>
    <div v-if="defaults.isShowMenus" class="treeMenus" style="">
      <el-button :disabled="canNotDoIt(sltParentId,[0])" type="primary" @click="add">新建</el-button>
      <el-button :disabled="canNotDoIt(sltParentId,[0,-101,-102])" type="primary" @click="edit">修改</el-button>
      <el-button :disabled="canNotDoIt(sltParentId,[0,-101,-102])" type="primary" @click="remove">删除</el-button>
    </div>

    <div class="treeContent">
      <el-tree :style="style" highlight-current node-key="id" class="filter-tree" :data="defaults.treeData "
               :props="defaultProps"
               @node-click="treeClick" :current-node-key="currentNodeKey" :default-expanded-keys="expandedKeys"
               :expand-on-click-node="false"
               :filter-node-method="filterNode" :lazy="defaults.asyn" :render-content="renderContent" :load="loadNode"
               ref="treeId">
    ...
    <!--删除弹窗-->
    <Modal :mask-closable="false" v-model="removeModal" height="200" class-name="vertical-center-modal" :width="400">
      <modal-header slot="header" :content="removeId"></modal-header>
      <remove v-if="removeModal" :deleteUrl="deleteUrl" @cancel="cancel" @remove="subCallback"
...
<style lang="scss">
  ...
  .hasSearch .treeContent {
    top: 36px;
  }

  .hasMenus .treeContent {
    top: 40px;
  }

  .hasSearchAndMenus .treeContent {
    top: 82px;
  }
    ...
    methods: {
      ...
      init() {
        ...
        if (isShowSearch && isShowMenus) {
          this.defaults.cls = ["hasSearchAndMenus"];
        } else if (isShowSearch) {
          this.defaults.cls = ["hasSearch"];
        } else if (isShowMenus) {
          this.defaults.cls = ["hasMenus"];
        } else {
          this.defaults.cls = [""];
        }

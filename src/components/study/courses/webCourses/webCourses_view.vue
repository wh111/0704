...
<template>
  <layout>
    ...
    <!-- 课件 -->
    <courseware-view v-if="menuActive === 'courseware'" :showProgress="true"
                     :saveProgress="saveProgress" @updateView="getViewData('courseware')"></courseware-view>
    <!-- 课程直播 -->
    <live-video-view v-if="menuActive === 'liveVideo'"></live-video-view>
    <!-- 评测与作业 (提交试题之后重新获取最新的数据并打开EQ菜单) -->
    <eo-view v-if="menuActive === 'EO'" :showInfo="!viewType" :isCourse="true"  @updateView="getViewData('EO')"></eo-view>
    <!-- 教学质量评价表 -->
    <tqv-info-view v-if="!viewType&&menuActive === 'TQVInfo'"></tqv-info-view>
    <tqv-info-view-teacher v-if="viewType&&menuActive === 'TQVInfo'"></tqv-info-view-teacher>
    <!-- 讨论区 -->
...
<script>
  ...
  import tqvInfoViewTeacher from '../../../teach/teacherResManagement/normCourse/TQVInfo/TQVInfo_view.vue'; // 教学质量评价表 查看
  ...
  export default {
     ...
     * saveProgress 是否上传进度
     */
    props: ['operailityData', 'showType', 'viewType', 'saveProgress'],
    data() {
      return {
        title: '---', // 课程名称
        logo: '', // 封面
    ...
    methods: {
      // 初始化
      init() {
        console.log('1', this.saveProgress)
        this.getViewData();
      },
      // 菜单点击
      menuClick(menu) {
      ...
      getViewData(menu) {
        ...
        this.ajax({
          ajaxSuccess: res => {
            ...
            setTimeout(() => this.menuActive = menu || 'notice', 1000)

...
<template>
  <div id="content" ref="content" class="modal">
    <el-form ref="formValidate" inline class="demo-ruleForm" align="right">
      <el-row style="margin-bottom:0">
        <!--列表操作按钮-->
        <el-col :span="10" >
          <el-button  class="but-col"  @click="add"  type="primary">添加</el-button>
          <el-button class="but-col" :disabled="disabled.publish"  @click="remove" type="danger">删除</el-button>
          <el-button  class="but-col" :disabled="disabled.publish" @click="publish" type="primary">发布</el-button>
          <el-button  class="but-col" :disabled="disabled.revocation" @click="revocation" type="warning">撤销</el-button>
        </el-col>
        <!--搜索项-->
        ...
        <el-table-column
          ...
          <template scope="scope">
            ...
            <el-button
              size="small"
              type="warning"
              :disabled="scope.row.volunteStatus=='YFB'"
    ...
    <div style="margin: 10px;">
      ...
      <Modal
        ...
        <operate v-if="joinModal" :type="'join'" :operate-data="joinData" @operate="subCallback" @cancel="cancel" ></operate>
        <div slot="footer"></div>
      </Modal>
      <!---->
      <!--取消弹窗-->
      <Modal
        ...
        <operate v-if="abolishModal" :type="'abolish'" :operate-data="abolishData" @operate="subCallback" @cancel="cancel"></operate>
        <div slot="footer"></div>
      </Modal>
      <!---->
    </div>
...
<script >
  /*当前组件必要引入*/
  //引入--新建--组件
  import add from "./volunteerActivity_add.vue";
  //引入--修改--组件
  import edit from "./volunteerActivity_edit.vue";
  //引入--查看--组件
  import show from "./volunteerActivity_view.vue";
  ...
  export default{
    ...
    methods: {
      ...
      handleSelectionChange(val) {
        ...
        val.map(item=>{
          if(['WFB','YJS'].includes(item.volunteStatus)){
            disabled.revocation = true;
          }else if(['YFB'].includes(item.volunteStatus)) {
            disabled.publish = true;
          }
        });

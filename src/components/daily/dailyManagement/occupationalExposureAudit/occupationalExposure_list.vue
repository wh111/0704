...
<template>
  <div id="content" ref="content" class="modal">
        ...
        <el-table
          ...
          <el-table-column
            ...
            <template scope="scope">
              ...
              <el-button size="small" :disabled="!['DSH'].includes(scope.row.status) " @click="audit(scope.row)">审核</el-button>
              <el-button size="small" :disabled="!['REPORTDSH'].includes(scope.row.status)" @click="medicalAudit(scope.row)">体检报告审核</el-button>
              <el-button size="small" :disabled="!['REIMBURSEMENTDSH'].includes(scope.row.status)"  @click="expenseAudit(scope.row)">报销审核</el-button>
            </template>
          </el-table-column>
          <el-table-column
            align="center"
    ...
    <div>
      ...
      <Modal
        width="800"
        v-model="expenseAuditModal"
        title="查看档案管理弹窗"
        class-name="vertical-center-modal"
        :loading="loading">
        <modal-header slot="header" :content="expenseAuditId"></modal-header>
        <expense-audit v-if="expenseAuditModal" @cancel="cancel" @expenseAudit="subCallback" :operaility-data="operailityData"></expense-audit>
        <div slot="footer"></div>
      </Modal>
      <!--查看档案管理弹窗-->
      <Modal
        width="800"
        v-model="medicalAuditModal"
        title="查看档案管理弹窗"
        class-name="vertical-center-modal"
        :loading="loading">
        <modal-header slot="header" :content="medicalAuditId"></modal-header>
        <medical-audit v-if="medicalAuditModal" @cancel="cancel" @medicalAudit="subCallback" type="audit" :operaility-data="operailityData"></medical-audit>
        <div slot="footer"></div>
      </Modal>
      <!---->
    </div>
...
<script>
  /*当前组件必要引入*/
  //引入--查看--组件
  import show from "../occupationalExposureWrite/occupationalExposure_view.vue";
  //引入--审核--组件
  import audit from "./occupationalExposure_audit.vue";
  //引入-- 报销审核--组件
  import expenseAudit from "./occupationalExposure_expenseAudit.vue";
  //引入--体检报告审核--组件
  import medicalAudit from "./occupationalExposure_medicalAudit.vue";
  ...
  export default{
    data() {
      return {
        ...
        deleteUrl:'occupationalExposureAudit/remove',
        reportedUrl:'occupationalExposureAudit/modifyStatus', //上报
        formValidate: {
          userName: '',  //学员姓名
          depId: '',      //科室ID
          status: ''      //审批状态
        ...
        // 这里获取当前登录人员信息
        applicant:{ applicantName:'张三' ,applicantId:'1'},
        operailityData:'',
        multipleSelection: [],
        dynamicHt: 100,
        self: this,
      ...
      //转换暴露状态
        ...
        listMessTitle:{
          ...
          ajaxParams:{
            url:'occupationalExposure/splist',
            params:{},
          }
        },
      ...
      converterStatus(status){
        switch (status){
          ...
          case 'REIMBURSEMENTBH' :status = '报销申请被驳回' ;
            break;
          case 'REIMBURSEMENTTG' :status = '报销申请通过' ;
            break;
        }
        return status;
      },
